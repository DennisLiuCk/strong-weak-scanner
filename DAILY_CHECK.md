...
2. **Actions 狀態**:`gh run list --workflow daily-fetch.yml -L 4`。正常每個交易日有三筆
   ...
   - 紅 → `gh run view <run-id> --log-failed`。常見:FinMind 限流(retry 已內建,
     ...
     → commit + push,隔天生效。
   - 即使綠的,也掃一眼 log 裡的 stderr 警告(`!` 開頭):price_adj 缺列、
     ...
     另確認最新資料日的 `oos_signal_snapshots` 檔數與 quality JSON 的 `eligible` 一致。
     正常交易時為 121；若有官方價格列可驗證的零交易股票，應顯示例如
     `universe=121、eligible=120、excluded=[3587]`，而不是把法人缺列補成 0。
     未附 exclusion 證據的 120/121 仍是錯誤，該日不得計入 OOS)。
     大盤正式值以 TWSE 官方含息報酬指數為主；FinMind 未發布只代表待交叉驗證，不阻擋
     TWSE 值。若儀表板／簡報顯示 `FinMind 備援`，表示該日 TWSE 官方值暫缺，需後續確認；
     雙邊同日值不一致會硬停。兩邊都缺時才會在衍生表重建前標紅，並先把已落地的五表／
     TDCC 當成「每日抓取進度（未完成）」保存。
