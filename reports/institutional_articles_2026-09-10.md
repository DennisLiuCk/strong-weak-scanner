...
- 文章：`MI-2026-09-10-GALLIUM-PERMIT-DELIVERY`，8 claims、3 documents及1 living
  index；圖譜`gallium-permit-delivery`只連砷化鎵生產、材料提純、目的地許可與交付查核。
  磷化銦需求未接成鎵需求，待驗證的收入遞延C8保留於帳本，未畫成已發生財務影響。
- MSCI官方原文日期7/16，模型資料截至5/10；作者及自家資料產品利益關係可定位。
  本文使用上游依賴框架，沒有採模型占比或推算短缺量。S&P另一份鎵草稿只有月份，
  不從檔名或PDF CreationDate猜日期，未採為正式來源；原選題watch不回寫。
- AXT 10-Q的SEC submission header核對filed 8/13、報告期6/30、主附件名稱及accession。
  7/30公司稿日期與季度也逐項核對。季報p.31–32分清提純鎵、GaAs/InP及用途；p.36
  區分亞洲歐洲一般獲批與對美GaAs尚未批准，InP則持續獲批與仍有待許可訂單並存。
  這是公司披露的歷史狀態，不作9/10即時調查或普遍法律結論；公司歸因沒有升為
  獨立因果實證。沒有計算新財務比率或市場統計，單公司定性案例不適用SE/t。
- 五份保存的實際web提取SHA256如下；它們不是原始HTML，未提取頁面與圖像不在
  離線核查範圍。三份研究文件之外，S4是活索引，Q1只證明S2的日期與附件身分。

| Payload | SHA256 |
|---|---|
| S1 MSCI原文 | `e7ce9ad0526d87536621c009c4178a017c7dfd3067cb2e961c5a7c503231ba30` |
| S2 AXT季報引用段 | `e6141631ea9fc1f60ca7a6715b304d9ceb12742d70c37f1e5f38455a1587c456` |
| S3 AXT公司稿 | `d5399ca131f402e18437fb1949ee0e8717bcc0858a863775852d017dcceb95e3` |
| S4 AXT申報入口 | `42b2f0c9e436ddc1a786b5e37bdd216180cc7b5a1a770ce3f32a56b064fa011b` |
| Q1 SEC submission header | `2284488c3152344f38f0dfe1c19acf242fb476ec45a4876ae831e8cfa7666ed8` |

- 第二位研究者在同一`material_evidence/` pack重算五份SHA、大小及0444模式並讀回
  引用段落，無阻擋問題；沒有重下載或把SEC託管當作SEC確認公司現況。AXT季報、
  公司稿與IR仍是同一消息鏈。QC見`tmp/institutional-articles-20260910/material_independent_qc.md`。
- 活索引只有表頭被提取，沒有動態申報列；已明示不可由空列判定沒有後續公告。
  正文採納交貨用字、模型白話及同美身分說明，裸露主張編號收回帳本，保留原文連結。
  新題加到政策與合規閱讀路線，兩項既有精確路線測試同步包含新圖譜，其他斷言保留。
- `material-publish-check`：macOS26.6.2 arm64、預設Python3.11.11（UTF-8 mode0，未設
  PYTHONUTF8）下692項測試、六lint及隔離build通過，正式DB與既有archive雜湊不變。
