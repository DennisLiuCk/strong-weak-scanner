...
上述第一閱讀層之後，候選既有的讀者線索、下一步、名詞與族群問句須收進單一原生
`.radar-reader-context`，首次載入預設收合；摘要只能報既有名詞數與正式族群問句數，不得另寫
研究結論。展開後必須逐字保留所有正式欄位、族群定位行動與推論邊界；同一工作階段的收合狀態可
跨雷達重繪與開文返回保留，但不得持久化或回寫 candidate／article／graph payload。研究查核區
必須保持獨立，不能把 source、priority、selection 或 evidence posture 混入這個讀者脈絡摘要。
...
路線與目前站次；下一站卡須先以原生 disclosure 摘要逐字顯示目前站與下一站的 graph／phase，
讓讀者先看懂「這一站 → 下一站」。展開後仍須完整顯示正式 route question、目前文章
`readingMission.keyPoints[0]`、兩步閱讀順序與下一站既有說明，不得摘要、刪除或重寫成新的結論；
收合時須讓下一站問題、站次與行動更早出現。桌機並排的自我檢查與下一站卡不得互相強制等高。
末站須明示只
...
一次」補回——重跑讀到的是另一天的批次。`full` 除了日期算術，還必須在**每個市場分別
實際觀測到窗口的每個日曆日**；僅有最後一天、兩端日期或跨市場聯集都不足。
`missingSpeechDatesByMarket` 列出缺日，包含週末；未知日不能推成零公告。
歷史 scan row／快照保持 append-only，未保存逐日證據的歷史 full 不代表今日已完成覆蓋。
批次已滾過窗口或只含部分日期時維持 `partial`，並在 coverage_note 說明缺口只能沿用
...
14. 重建研究中心並檢查可信度、知識圖譜財務面板與 deep link 的桌機／行動版顯示。首次進入、
    ...
    重點」且預設收合，下一站問題逐字取自下一篇 route 主文章。非末站是否先以可鍵盤操作、至少
    44px 的原生 summary 逐字顯示目前站與下一站的 graph／phase label；完整脈絡是否預設收合，
    展開後仍保留同一兩步 ordered list、正式 route `question`、目前文章第一個 key point 與下一站
    既有 description，並明示這只是閱讀順序、不代表供應鏈、受惠或因果關係；收合後下一站問題、
    站次與 CTA 是否可在較短距離內看見；不得用題名、相似度或模型補 label、question 或建立 edge。
    末站完成卡是否逐字收回
...
16. 在文章間切換與由雷達／圖譜開啟文章時，確認閱讀區立即回到頂端；雷達已升格候選開文後，
    ...
    雷達候選卡另在 320px、390px、884px 與 1280px 驗收正文橫跨完整卡寬、卡頂可直接讀到
    「研究順序 N」及「只排研究待辦，不是股票或投資排名」，並確認讀者看到的是白話 status 與
    「閱讀這題的文章」行動；「看目前線索與產業角色」首次載入須收合，摘要列出的名詞／族群數
    要等於同卡正式資料，展開後完整內容與族群行動都仍存在。開文返回須保留該卡收合狀態與焦點；
    340px 以下兩個主要行動改成 44px 單欄，其他寬度可同列但不得把中文拆成孤字。頁面不得水平
    溢出，原始 status label、rank 與 payload 不得因顯示翻譯或收合改寫。
    桌機與行動版均檢查
