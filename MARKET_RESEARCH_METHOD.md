...
`radar` 來處且目前文章仍是該候選自己的升格 article ID；換到下一站文章後不得沿用。
...
14. 重建研究中心並檢查可信度、知識圖譜財務面板與 deep link 的桌機／行動版顯示。首次進入、
    ...
    開始操作後導覽即隱藏，且行動版不得產生水平溢出。進入知識圖譜後，必須先用白話說明
    ...
    提供清楚的完成與返回路線狀態；不得改用相似度或熱門度排序。每個路線
    ...
    相似度或插入未命中的詞。新手段落、研究摘要與查核附錄不重複插入。研究摘要須以「一句話
    ...
    從文章的圖譜卡開圖時，首屏是否明示剛才文章與下一站，返回後是否恢復同一延伸卡位置、焦點
    及原本矩陣起點；關係解讀末端是否也能用至少 44px 的按鈕回到同一位置並繼續下一站；直接圖譜
    ...
    回到頁首。另逐一核對非 route 的文章推薦卡：必須顯示由兩篇現有 `stockIds` 交集產生的共同
    ...
    文章頂端；同一族群跨卡重複須明示為多個系統角色，不得暗示重複計分。三層完成度說明預設
...
16. 在文章間切換與由雷達／圖譜開啟文章時，確認閱讀區立即回到頂端；雷達已升格候選開文後，
    ...
    直接文章 deep link 不得顯示雷達來處。桌機與行動版均檢查
