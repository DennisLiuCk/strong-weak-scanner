...
路線與目前站次，末站須明示只完成閱讀順序、不代表研究結論完成，且返回圖譜時須從頂端開始。
...
1. 清單卡與文章頁首是否先用 `config/research_topic_guide.csv` 的人工中文問題交代本文要解開
   ...
   回寫原 topic、替換閱讀任務或被誤當成 claim／source。標題與查核警語後是否再用「新手閱讀任務」
   ...
   沒有問題時是否退回舊格式，而且沒有從正文、題名或搜尋字生成替代問句。
   ...
   `H#` 假說與 `YYYYH#` 半年是否分開，原 token 與原句是否完整保留，沒有符號時是否不顯示
   空入口。展開摘要是否至少 44px，Enter／Space 都能切換；80 字以上重點是否只在既有中文
   分號或句末標點後加入零文字停頓，而沒有改寫、截斷或重排字詞。
   ...
   原始定義，沒有命中時是否不顯示空容器；同節命中達 9 個以上時是否只先顯示原字典順序前 6 個，
   其餘由可鍵盤展開的原生 `details` 完整保留，並明示排序不代表重要性；8 個以下是否維持全部
   直接顯示，且兩種狀態都不得刪詞、改序、另寫定義或改動原文；一般市場議題進入第一個主正文
   ...
   族群、graph 或 route 另寫問題；市場議題主正文若有 `n` 個
...
14. 重建研究中心並檢查可信度、知識圖譜財務面板與 deep link 的桌機／行動版顯示。首次進入、
    ...
    保留 `maturity-route` 返回脈絡。讀者手動收合才可保存為本地顯示偏好，resize 不得覆寫；
    ...
    都能直接開文並回到頁首。單一圖譜入口須標為「看這站證據關係」，不得冒稱完整路線。末站須
    ...
    相似度或插入未命中的詞。同節命中至少 9 個時只先呈現原字典順序前 6 個，其餘放入原生
    `details` 且展開後一個不少；8 個以下全部直接顯示。新手段落、研究摘要與查核附錄不重複插入。研究摘要須以「一句話
    ...
    因果關係；不得用題名、相似度或模型補 label 或建立 edge。缺任一必要項即停止建置。回看操作
    ...
    驗證動作解釋每個候選；
    active 候選的頂部地圖是否逐一且同序枚舉全部 `reader_question`、固定 status 讀者標籤與
    `next_check`，每個 `aria-controls` 是否唯一對到同候選卡，點擊後捲入並聚焦正確卡片；桌機
    四欄、中幅兩欄、窄幅單欄且首次預設收合，展開後不得水平溢位；卡尾是否能回到並聚焦同一
    地圖。地圖是否只縮短找題路徑，
    沒有改寫 rank、候選內容、研究判定或投資邊界。
...
16. 在文章間切換與由雷達／圖譜開啟文章時，確認閱讀區立即回到頂端；雷達已升格候選開文後，
    ...
    44px 原生摘要一次展開，問句、族群順序與補充內容不得因 responsive 改寫。快讀與閱讀任務
    只能由既有結構化 register／正文 blocks 合成，不得另寫一套無 claim ID、source ID 或 monitor
    支撐的結論。另各抽查一般文章至少一個達 120 字、含兩個以上句末的主正文段落，以及市場議題
