...
def final_pass_ready(end, now=None):
    """當日 final pass 只在台北 23:40 後開放；明確回補過去日期不受限。"""
    ...
    if end > today:
        return False
    return (current.hour, current.minute) >= FINAL_PASS_READY_HM
...
def fetch_missing_raw(con, ids, ds_list, start, end, token, sleep=0.25,
    ...
    # 23:47 終版重跑仍會再次刷新 holding，不會讓早場版本被誤認為完成。
...
def main():
    ...
    ap.add_argument("--raw-only", action="store_true",
                    help="只落地原始表 checkpoint,不抓事件、不重算 metrics(提前場用)")
    ...
    current_taipei = taipei_now()
    today_taipei = current_taipei.date()
    end = args.end or today_taipei.isoformat()
    start = args.start or (today_taipei - timedelta(days=args.days)).isoformat()
    if args.final_pass and not final_pass_ready(end, current_taipei):
        sys.exit(
            f"拒絕過早 final-pass:{end} 當日終版門檻為台北 "
            f"{FINAL_PASS_READY_HM[0]:02d}:{FINAL_PASS_READY_HM[1]:02d}；此前只能落地 checkpoint")
