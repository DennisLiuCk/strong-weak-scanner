...
2. **最近研究**：首頁只保留正式筆記、多空小作文、市場議題三類入口；事件錨點歸入市場議題，
   ...
   中文句末標點時，只要實際 `.article-section` 內容欄不超過 860px，就會在原句末加入視覺停頓；
   因此 1280px 視窗中的 836px 專注閱讀欄不會因整體 viewport 較寬而把句子重新黏成一塊。1180px
   以下保留較明顯的 0.55em 停頓，其餘符合容器門檻的正文用 0.45em；仍是同一個段落、原字序與
   閱讀順序，不是摘要、改寫或新增主張。
