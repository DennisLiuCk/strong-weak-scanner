...
2. **最近研究**：首頁只保留正式筆記、多空小作文、市場議題三類入口；事件錨點歸入市場議題，
   ...
   文字，指出這篇先釐清的情境或問題、讀完應能回答什麼，不另外生成研究摘要或提高語氣。「新手先讀」
   的畫面順序改為三句重點、為什麼重要、預設收合的名詞小字典、接下來怎麼追、想一想，讓讀者
   先抓主線並直接看到追蹤與反證問題，只有遇到陌生詞時才展開完整字典；再由緊接的研究摘要用
   ...
   對應卡捲入畫面。預設篩選下的「第一次來」導覽也會重新顯示，不會因曾開過文章就永久隱藏。
   讀完後，「從這篇接著學」顯示下一個主題站；末站則明示路線已讀完並帶回知識圖譜頂端。例如先
   辨識液冷產品資格，再拆迴路責任邊界。手機首次進入圖譜時，「新手讀圖」只保留四步讀法摘要，
   ...
   既有路線與站次，不是熱門度或投資排序。矩陣頂端先把四條既有學習路線改寫成四個系統問題，並逐卡列出路線
   主文章實際宣告的相關族群與第一站；同一族群出現在多張卡，只表示它參與不同系統問題。
   問題卡不靠相似度新增關係，點擊即可從該路線第一站開始。讀完後再展開「完成度怎麼看」，用
