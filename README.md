...
2. **最近研究**：首頁只保留正式筆記、多空小作文、市場議題三類入口；事件錨點歸入市場議題，
   ...
   閱讀就先穿過三個輔助區。市場議題的「開始讀三句重點」直接把鍵盤焦點送到同篇三句重點；正式
   ...
   緊接在市場議題任務後，三句卡結束就直接銜接本文族群角色與所在學習階段。文章日期／閱讀時間放在這段定位之後，
   ...
   共用同一組 section index 與 scroll-spy，不另生成段落摘要或研究結論。
   ...
   同一候選原本為各族群寫明的「在這題要回答什麼」，只在該候選直接開啟自己的升格文章，或先經
   同一候選的矩陣族群入口再開啟該文時，逐字帶到相符的產業角色卡；讀者先對回本題分工，再看
   族群平常負責什麼與不能外推到哪裡。矩陣路徑的文章首尾保留族群、雷達題次與同一問句，返回時
   回到原矩陣列而不是跳回通用族群入口。
   直接文章連結、一般清單入口與沿路線換到下一站文章都不帶入這些候選問句，避免把前一題脈絡
   錯套到另一篇文章。
   ...
   虛構來源。文章前段另以「再把產業角色分開」重用正式族群的白話角色與混淆邊界；一至四個角色
   直接用卡片同時比較，不再要求讀者切換按鈕並記住上一張。超過四個角色時先保留一張起點卡，
   其餘角色放進一次即可全部展開的原生區塊。直接連結進文也能在三句重點後比較本文實際宣告的
   族群，再讀技術細節；卡片並列不代表上下游、受惠或投資排序。未宣告族群的文章則不顯示、不靠
   ...
   並可從第一篇開始或定位完整族群進度。四個系統問題逐卡列出路線主文章實際宣告的
