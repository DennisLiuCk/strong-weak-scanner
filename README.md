...
台股半導體三族群(**被動元件 / 功率元件 / 封測**,名單見 `config/universe.csv`,現 39 檔)
...
- **Universe 治理**(季度,工具 `scripts/screen.py` + `config/candidates.csv`):
  R1 業務歸屬(主營收 >50% 屬族群業務,人工覆核)/ R2 市值遲滯(≥50 億納入、
  現有成員 <30 億才剔除,防反覆進出)/ R3 近 20 日中位成交值 ≥3,000 萬 /
  R4 上市 ≥60 交易日。變更後全歷史按新名單重算(族群中位與排名都會變),
  審計軌跡以 CHANGELOG 為準。
