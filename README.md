...
2. **最近研究**：首頁只保留正式筆記、多空小作文、市場議題三類入口；事件錨點歸入市場議題，
   ...
   驗證動作說明研究目的；原始 `why_now`、下一份證據、反證條件與來源完整收在查核區。這是
   研究資源排序，不是投資評分。知識圖譜則用同一
   ...
   證據邊界與下一個驗證節點。「族群矩陣」的每列先提供一篇既有學習路線主文章作為族群起點，
   也能另看該族群全部文章；起點優先使用把該族群列在第一位的文章，再沿用既有路線與站次，
   不是熱門度或投資排序。矩陣頂端先把四條既有學習路線改寫成四個系統問題，並逐卡列出路線
