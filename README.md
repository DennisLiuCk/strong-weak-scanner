...
2. **最近研究**：首頁只保留正式筆記、多空小作文、市場議題三類入口；事件錨點歸入市場議題，
   ...
   的查核狀態與觀察層警語。第一次進入研究中心可照「先看族群矩陣 → 再讀市場議題 → 最後追
   知識圖譜」三步建立全貌；這是閱讀順序，不是投資排名。點開文章後，桌機預設進入專注閱讀；
   標題與查核警語後先顯示「新手閱讀任務」，直接重用本文「為什麼重要」與「想一想」的既有
   文字，指出這篇先釐清的情境或問題、讀完應能回答什麼，不另外生成研究摘要或提高語氣；卡片
   內的「開始讀三句重點」會直接把焦點送到同篇既有重點，不必先穿過產業角色、metadata 與路線
   輔助資訊。想先比較本文族群的讀者仍可順著卡片往下看「新手產業定位」。「新手先讀」
   的畫面順序改為三句重點、為什麼重要、預設收合的名詞小字典、接下來怎麼追、想一想，讓讀者
   先抓主線並直接看到追蹤與反證問題。三句重點改用逐句卡片；每句先比對同篇名詞小字典，卡片
   ...
   呈現既有文字，讓讀者先抓結論、再檢查邊界，而不是在同一層五點清單裡找標籤。讀到正文後段
   若忘記術語，行動版在原字典捲出畫面後顯示左下「名詞速查」，桌機則固定放在本頁大綱下方；
   ...
   需要逐欄對照的 ledger 改成卡片。
   正文結束後，「從這篇接著學」先重問首屏的「想一想」，讓讀者用自己的話回答；需要提示時
   才展開逐字取自同篇「三句話抓重點」的三點內容，也能回到原段落。下一站卡同時顯示下一篇
   既有文章要回答的問題與站次，再連到同公司文章、既有知識圖譜或同族群研究，幫新手把
   單篇內容接成學習路徑。延伸卡只使用現有文章、具名公司、宣告族群與
   可追溯圖譜關係，不會因關鍵字相似就新增供應鏈主張；新手導讀先寫中文概念，再在括號保留
   英文術語，並用「範圍、角色、成熟階段」拆解技術新聞。完整來源、主張、比較與追蹤表保留在
   可展開的研究查核附錄。研究中心的
   「研究雷達」以「下一步研究什麼」呈現，卡片正面先用一個白話問題、2–4 個關鍵詞與下一個
   驗證動作說明研究目的；候選明列的 1–4 個正式族群各自附一個「在這題要回答什麼」的白話
   問句，讓讀者先分清零件、子系統、整機或製程等研究責任，再點擊定位族群矩陣的起讀文章、
   已完成、最大缺口與下一步。矩陣會保留同一問句並可返回原候選。這些問句只拆分研究範圍，
   不代表上下游順序，也不等於族群、公司或題材已被證實受惠。原始 `why_now`、
   下一份證據、反證條件與來源完整收在查核區。這是研究資源排序，不是投資評分。知識圖譜則用同一
   證據底層分開查看「公司曝險」與「產業依賴」。新手先依「供電與散熱、記憶體與
   ...
   族群矩陣的系統問題卡與文章內的站次定位都提供可展開路線地圖：逐站顯示既有 graph 名稱、
   同篇「想一想」問題與閱讀時間，目前文章另標成「目前這站」；每站可直接開啟對應文章，
   「看這站證據關係」才會前往該站圖譜。路線地圖只取 route `graphIds`、各 graph 第一篇既有
   `articleIds` 與該文 `readingMission.question`，不另寫站點結論或推導跨族群關係。
   ...
   虛構來源。文章前段另以「再把產業角色分開」逐一重用正式族群的白話角色與混淆邊界；直接連結
   進文也能先比較本文實際宣告的族群，再讀技術細節。超過四個族群時預設收合角色切換，未宣告
   族群的文章則不顯示、不靠文字猜測。預設篩選下的「第一次來」導覽也會重新顯示，不會因曾開過
   文章就永久隱藏。
   ...
   手機首次進入圖譜時，「新手讀圖」只保留四步讀法摘要，
   「目前位置」則直接顯示學習路線、中心主題、投影視角與已選證據；兩區都能原位展開完整內容，
   讓主題說明與起讀動作先進入首屏。展開後主題仍用單一下拉選擇，不先顯示全部按鈕牆；切換路線、
   視角、證據或台股範圍時，目前位置同步更新，不另造圖譜狀態。
   ...
   既有 `articleIds`，不另外生成摘要、公司連結或受惠結論。
   ...
   起點，也能另看該族群全部文章。指南逐一覆蓋 `config/groups.csv` 的正式族群，只是讀者分類，
   不會建立上下游順序、公司認證或受惠方向；起點則優先使用把該族群列在第一位的文章，再沿用
   既有路線與站次，不是熱門度或投資排序。矩陣頂端先提供「先認識一個族群」：點 11 個正式
   族群之一，即時查看同一份白話角色、混淆邊界、既有路線收錄情形，並可從第一篇開始或定位
   完整族群進度。接著再把四條既有學習路線改寫成四個系統問題，逐卡列出路線主文章實際宣告的
