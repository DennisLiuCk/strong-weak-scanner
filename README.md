...
2. **最近研究**：首頁只保留正式筆記、多空小作文、市場議題三類入口；事件錨點歸入市場議題，
   ...
   標題與查核警語後先顯示「新手閱讀任務」，把本文既有第一句「三句話抓重點」與「想一想」
   第一題分別放進「先抓住這一句／讀完能回答」，不另外生成研究摘要或提高語氣；原本較長的
   「為什麼重要」開場完整保留在可展開說明，需要更多脈絡時再看。這兩句若命中同篇名詞小字典
   或研究中心共通語，卡片會先顯示預設收合的「先認得這兩句的 N 個詞」；定義逐字沿用既有來源、
   同詞只列一次，完全沒有命中時不顯示空提示。卡片內的「開始讀三句重點」
   會直接把焦點送到同篇既有重點；完整三句重點緊接在任務後，三句卡結束就直接銜接本文族群角色
   與所在學習階段，不再先穿過重要性、字典、追蹤與反證問題。文章日期／閱讀時間放在這段定位之後，
