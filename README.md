...
- `原始資料規劃:FinMind F 次;TWSE/TPEx 價格批次 P 次` 中，`F` 只計
  `inst`／`margin`／`holding`／`sbl` 四張原始表；除權息、分割、TAIEX、參考個股
  另列在下一行 `事件 API E 次`，本輪 FinMind 邏輯請求總數是 `F+E`。以 universe
  檔數 `N` 計，完整新交易日的理論值為 `4N + N + 1 + 1 + 2 = 5N+4`；目前
  `N=121` 即 609。這是邏輯請求數，不含 HTTP retry；歷史缺口會額外增加請求。
- 官方價格是每個「待探測或待補日期」TWSE、TPEx 各一次，所以正常新交易日 `P=2`。
  `P=0` 表示 SQLite 價格已完整而被智慧補缺跳過，**不是**兩個官方端點失敗，也不能拿
  這輪綠燈宣稱 GitHub runner 已實際測到新價格路徑；要等真正的新交易日或既有價格缺口。
...
- `0 rows · 1 requests` 不必然是 API 故障：停牌、上市前或來源本來沒有該歷史欄位時，
  缺口會在預設 15 日視窗內繼續嘗試，直到來源補出或日期滑出視窗。先定位
  `stock_id × table × date`，再決定是否需要 coverage／例外規則，不要把空回應直接補 0。
