...
2. **最近研究**：依文章 metadata 顯示最近 14 個日曆日更新的正式筆記、多空小作文、
   市場議題與事件錨點；預設顯示最近 12 篇，可展開全部並直接開啟 GitHub 全文。新近不代表
   證據較強，畫面會保留正式筆記查核狀態與小作文／候選議題的觀察層警語。
3. **台積電專區**：閱讀上游價格、外資持股、月營收與法說指引；這一區全為觀察層。
4. **族群比較**：四象限看價籌位置與 5 日位移，熱圖比較各欄名次，排行榜逐欄排序；三個視圖
   使用同一批資料並聯動高亮。相對最好不等於原始值已轉正。
5. **個股分層**：可按族群篩選、展開所有成員，並查看近 5 日已確認分層的變化。
6. **族群內個股**：搜尋或單維排序；七因子列可展開原始值、門檻量尺、權重與加權貢獻。
