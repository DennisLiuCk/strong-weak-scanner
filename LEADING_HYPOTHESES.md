...
逐則 H# 是原子化的可證偽主張；「多空觀點」是報告層的合成敘事,回答讀者真正的問題:
「把所有假說、正式筆記與量化數據放在一起看,多方與空方各自最強的故事是什麼?」
章節放在 H# 之前,含 `narrative_meta`(`narrative_version: 1`、`narrative_updated`、
`quant_context_as_of`)與三個固定小節,存在即受 lint 檢查;2026-07-12 前的回溯基線
不回溯補寫,2026-07-18 起的前瞻捕捉未附時 lint 以 warning 提示：

- **看多小作文／看空小作文**：兩篇刻意對立的敘事,各 100–800 字。只能使用 H#、
  正式筆記與量化背景已有的材料,必須各自引用至少一則現有 H#,並以「最脆弱處」
  一句自陳本方敘事最容易被拆掉的點——沒有反身檢查的小作文只是行銷文。
- **勝負手**：1–3 條 bullet,列出「什麼即將可觀測的資料或事件會裁決多空」,
  對應 H# 的可證偽條件與 `review_due`;誰對誰錯由資料裁決,不由文筆裁決。
- **量化背景**：`leading_hypotheses.py --context <股號>` 從自家 db(月營收、價格
  動能、族群內排名、外資/借券/融資、TDCC 大戶、處置注意旗標、台積電法說族群指引)
  產生的快照,貼上後可裁剪,但標題截至日期與警語「不得作為生命週期轉移證據」
  必須保留。觀察層數據僅供敘事語境與捕捉觸發——小作文再有說服力,生命週期轉移
  仍只認正式文件、可重算實績與事前指定的里程碑。
