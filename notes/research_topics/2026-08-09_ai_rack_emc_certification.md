...
- **Pre-compliance**：正式送驗前以較小場地、探棒、模擬或子系統測試先找出風險；它可降低重測機率，但不能冒充正式合規結果。
- **Chamber／turntable**：隔離外界雜訊的測試空間與旋轉被測設備的平台；機櫃太大、太重或需要兆瓦級供電時，傳統實驗室硬體可能根本放不下或帶不動。
...
- OCP 2026 訪談由 Meta 的 AI hardware EMC 負責人指出，解耦式、重型、兆瓦級 AI 機櫃讓 chamber 尺寸、turntable 承重、供電與 lab capacity 同時成為送驗瓶頸。
- IEC CISPR 32 與 FCC measurement／accredited-firm 入口能定位設備排放與測試責任，但 FCC 也明說名單本身不保證某家實驗室能量特定裝置。
- 因此元件有 EMI 抑制功能、子系統做過 pre-scan、設備符合某標準與整櫃能被正式測試是四件事；2327 國巨目前只接到第一層產品能力，沒有 AI rack qualification 或財務映射。

### 為什麼重要

一般伺服器可以把單機送進既有 chamber，但 AI 系統逐步把 compute、power 與 liquid cooling
拆到多個 rack／sidecar，再以高速纜線與高壓供電組合。此時最容易犯的錯，是看到某顆
common-mode choke、吸波材或 PSU 通過元件規格，就假設整櫃合規問題已解決。真正的責任鏈是：
零件抑制哪個雜訊路徑、子系統如何預掃、最終被測 equipment configuration 是什麼、哪個
lab 能提供尺寸／承重／供電與量測能力、失敗後誰重設計。把這些層分開，才能判斷瓶頸究竟
落在被動元件、電源、機構／ODM、測試服務還是系統架構，而不是把「EMC 很重要」直接翻成
某一族群受惠。

### 接下來怎麼追

- 2026-08-31 重查 OCP Test and Validation／podcast、IEC 與 FCC 入口，尋找具名 full-rack test method、被測配置、accredited scope、lab power／weight／chamber 能力與 lead time。
- 國巨 2026Q2 法說後檢查是否首次把 KEMET／Pulse 的 EMI 元件連到 AI rack、具名 qualification、production part、客戶與可辨識收入；只重複 AI 終端成長不算。
- 等待 hyperscaler 或 ODM 公開模組證書能否沿用到解耦式整櫃、哪些配置變更會觸發重測，以及 simulation／pre-compliance 能替代到哪個邊界。

### 想一想

- 如果每個 compute tray、PSU 與 cooling sidecar 都各自有合規文件，整櫃重新接線與同步運作後的輻射路徑真的會保持不變嗎？
