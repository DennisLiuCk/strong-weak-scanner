...
一般伺服器可以把單機送進既有隔離測試室，但 AI 系統逐步把運算、供電與液冷拆到多個機櫃
或側掛設備，再以高速纜線與高壓供電組合。原本能測單機的實驗室，未必放得下、帶得動或
供得起完整機櫃。

最容易犯的錯，是看到某顆共模扼流圈、吸波材料或電源供應器通過元件規格，就假設整櫃合規
問題已解決。元件測試只回答該零件能處理哪種雜訊，不能回答重新接線、裝進機構並同步運作後
的整機排放。

真正的責任鏈要逐層問：零件抑制哪條雜訊路徑？子系統如何預掃？最後送驗的設備配置是什麼？
哪間實驗室能提供尺寸、承重、供電與量測能力？失敗後由誰重設計？把這些層分開，才能判斷
瓶頸落在被動元件、電源、機構／系統組裝、測試服務還是系統架構，而不是把「電磁相容很重要」
直接翻成某一族群受惠。

讀者可以把證據放進三個責任盒：元件層回答「能抑制哪種雜訊」；設備層回答「指定配置是否
符合排放要求」；實驗室層回答「場地是否真的能承載並量測完整配置」。一份文件若只放得進
其中一盒，就不能替另外兩盒下結論。
