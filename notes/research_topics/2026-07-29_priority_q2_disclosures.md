...
- **Evidence pack**：把實際引用文件、頁碼與檔案雜湊固定下來，讓另一位 reviewer 能離線重算的證據包。
...
同樣寫著「Q2」的連結，可能是完整季報、法說簡報、三頁摘要、董事會預告，甚至只是空的文件清單。若不先辨識證據角色，讀者很容易把事件存在誤當成數字成立，或把 NRE 的 AI 開發占比誤當成量產收入。這次漏掃也證明：公司 IR 頁面不是唯一入口，研究流程本身同樣需要被反證與修正。

### 接下來怎麼追

- 先掃交易所正式數值／事件索引，再查 MOPS 直接附件與公司 IR；看到 OpenAPI 數值列時，仍要確認完整 PDF 是否存在。
- 文件可下載後，確認期間、頁碼、單位與是否含會計師核閱報告及附註。
- 新附件上線後封存到 evidence pack，由另一位 reviewer 重算；只看到標題時保持 `review_due`。
...
- AI 占 NRE 35.4% 卻只占 MP 0.4% 時，你會要求哪一種 cohort、流片、驗證或量產證據，才願意相信開發案正在轉成收入？
...
- [智原官方法說頁](https://www.faraday-tech.com/tw/html/IR/QuarterlyResults.html)、
  ...
  另顯示 `202602_3035_AI1.pdf` 於 7 月 28 日 16:07:02 上線；本輪只驗證索引存在，未把
  完整季報封存成 evidence pack，故不宣稱附註已重算。
