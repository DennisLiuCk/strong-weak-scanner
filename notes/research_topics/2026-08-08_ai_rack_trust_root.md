...
- **Caliptra**：一份開放規格，規定晶片內部要放一小塊「保鑣電路」，開機時逐段檢查晶片自己要跑的程式有沒有被掉包。它由 OCP（開放運算計畫，雲端業者與硬體商共同制定資料中心規格的組織）發布，內容包含規格、電路設計與韌體，不是某一家公司的產品。信驊產品頁把這塊電路寫成 SiRoT。
- **量測與 attestation（證明）**：保鑣電路開機時把每一段韌體算成一組指紋記下來，這叫量測；把指紋加上簽章交給外部系統查驗，叫 attestation。**簽章只保證這段說法沒被竄改，不保證裡面那一版韌體是安全的**——這是最容易誤讀的一點。
- **SPDM**：零件之間的一套「對話規則」（規格編號 DSP0274，由標準組織 DMTF 維護），讓機器裡兩個零件互相確認身分、交換「我現在跑的是哪一版」的證據，像零件之間的驗證身分證流程。它另有一份獨立的授權規格，處理「確認你是誰之後，你到底可不可以做這件事」。
- **一致性測試（conformance test）**：用一套共同的測試題目，驗證某個產品真的照規格做了。沒有它，「支援某規格」就只是廠商自己說的。SPDM 這套題目的指引（編號 DSP-IS0023）到現在仍掛在「草稿」區，標準組織把這種未定稿狀態寫作 work-in-progress，簡稱 WIP。
- **OCP S.A.F.E.**：OCP 推的安全稽核制度（Security Appraisal Framework and Enablement）。由核可的第三方稽核機構（Security Review Provider）檢查裝置的硬體與韌體，把報告掛在該裝置旁邊，讓每個買方不必各自重測一次。
- **BMC**：伺服器裡一顆獨立於主系統的管理晶片。主機關機或當掉時它仍在運作、仍能下指令，所以它自己可不可信特別關鍵。
- **AST2700**：信驊（5274）的第八代 BMC 晶片，產品頁載明內建 Caliptra 保鑣電路。DPE 與 Redfish 等其餘縮寫只出現在帳本引用，讀正文不需要記。

### 三句話抓重點

- 要讓一道「把這櫃機器斷電斷水」的指令可信，得先回答三個問題：你是誰、你現在跑的是哪一版、你可不可以做這件事——這三題現在都各有一份已發布的公開規格可循。
- 但還有第四題沒人替買方回答：**我憑什麼相信這家廠商真的照規格做了？** 晶片這一側已經有第三方稽核制度在運作，零件對話那一側的測試題目卻還停在 2022 年的草稿。
- 所以「支援 Caliptra／SPDM」目前只能讀成廠商宣稱有做，不能讀成已被獨立查證；台灣唯一直接對應的個股 5274 信驊，產品頁列了這項功能，但沒有任何出貨量、客戶認證或財務數字可以引用。

### 為什麼重要

先想一個具體場景。資料中心裡有一櫃 AI 伺服器過熱，管理系統送出一道指令：關掉這一櫃的冷卻液、切掉電源，讓維修員進去換零件。這道指令會讓一整櫃上千萬的機器停機。**系統憑什麼相信送出這道指令的，真的是有權限的管理程式，而不是有人冒名？**

上一篇文章把機櫃控制拆成「誰有權發請求、誰做安全裁決、動作結果怎麼回報」。那條鏈預設了一件事：發請求的那個身分是真的。如果身分本身可以偽造，後面所有安全護欄與稽核紀錄，都只是忠實記錄了一個假名做過什麼。這篇處理的正是那個被跳過的前提。

真正值得學起來的判讀方式只有一句：**把「規格寫好了」和「有人能替你查證」分成兩件事看**。產業新聞幾乎只講前者——某晶片「支援 Caliptra」、某平台「採用 SPDM」。但安全要變成商業價值，靠的是後者：買方能不能不聽賣方自述就確認這件事。這個落差可以觀察、也可以被推翻，比爭論「AI 資安算不算題材」有用得多。

### 接下來怎麼追

- 那份一致性測試指引（DSP-IS0023）是否從草稿轉成正式版本，以及配套的授權白皮書是否如標示在 2026 年第三季出現。這是本篇最直接的反證點。
- OCP 是否公布新的 S.A.F.E. 稽核報告或新增稽核機構，特別是稽核範圍有沒有從晶片延伸到 BMC 與整櫃層級。
...
回到前面的場景。一道「關掉這櫃的水電」的指令要能被信任，得依序過四關。前三關各有一份已發布的公開規格，第四關才是缺口。

**第一關：你是誰。** 晶片裡那塊保鑣電路在開機時建立自己的身分，並把每一段韌體算成指紋。Caliptra 就是這一關的規格（C1）。值得注意的是它刻意只做最小的事，還明白把「認證」這件工作寫在自己範圍之外（C2）——換句話說，規格從一開始就假設查證由別的制度負責。這不是漏洞，是分工，但也代表你不能指望這份規格替你擔保什麼。

**第二關：你現在跑的是哪一版。** 光有身分不夠，還得把指紋交出去讓對方驗。SPDM 規定零件之間怎麼提供憑證、怎麼在運作中重新認證、怎麼取得韌體與設定的量測值（C4）。晶片裡那塊保鑣電路可以充當這一關的簽章來源（C2），這正是兩份規格接起來的地方。

**第三關：你可不可以做這件事。** 「證明你是誰」和「允許你做什麼」是兩件事。SPDM 體系把後者放在另一份文件，2025 年 12 月已正式發布（C3）。順序本身就說明了問題：能證明身分的規格，比能決定權限的規格早了好幾年。

**第四關：我憑什麼相信廠商真的照做了。** 這一關沒有規格能回答，只能靠查證制度。晶片這側有 OCP S.A.F.E.：核可的第三方機構檢查裝置、出具報告（C1）。零件對話那側的一致性測試指引，到本輪查看時仍是草稿，日期停在 2022 年（C3）。兩邊都存在，但一邊在運作、一邊還沒開始。

**接回上一篇。** 上一篇談的是「誰有權發請求、誰做最終裁決」，那個分辨動作預設身分是真的——前三關補的就是這個預設。但本篇只把這個銜接寫成推論（C8）：沒有任何文件說明，某台機器驗證失敗時，那道斷電指令到底會被拒絕、會進入安全狀態，還是只留一行紀錄後照常執行。那仍是未驗證的 C9。
...
反向也要說清楚：那份指引長期停在草稿，並不證明 SPDM 不安全或沒人採用，只證明買方目前缺少一條標準化的第三方查證路徑。把前者說成後者是本篇最容易犯的錯，因此 C7 的界線明確排除對產品實際安全性的評價。

## 這篇對個股判斷的用處與界線

先說結論：**這篇現在不支持任何個股動作。** 唯一直接對應的 5274 信驊，能引用的只有產品頁上一行功能敘述；沒有出貨量、沒有客戶認證階段、沒有拆分的收入或毛利。把「產品頁列了安全功能」讀成訂單或毛利改善，正是本系統一再要避免的錯誤，因此它在帳本裡是未驗證的 C10，在族群影響裡也只給 watch。

那它的用處是什麼？是一把尺。接下來一定會出現「某某晶片支援 Caliptra」「某某平台採用 SPDM」這類敘述——包含法說會與新聞稿。有了這篇，你可以對每一則這樣的說法追問三個具體問題：

1. 它講的是哪一關？是晶片能證明自己（第一關）、零件之間能互驗（第二關），還是真的做到權限判斷（第三關）？三者價值完全不同，但廠商通常混著講。
2. 有沒有第四關？有沒有第三方稽核報告，或任何買方端的驗收條款，而不只是廠商自述？
3. 有沒有錢的足跡？是產品頁上的一行規格，還是已經出現在出貨、客戶認證階段或財務數字裡？

這三問可以直接套用在任何一家宣稱做 AI 機櫃安全的公司，不限於 5274。當某家公司第一次能回答第二問和第三問時，那才是這個題目從技術框架轉成投資判斷的時點——而那個時點什麼時候到、會不會到，本篇不做預測。
