...
- **製程控制**：在製造途中用量測、檢查與缺陷分析找出偏差，再把結果回饋到製程調整；它不同於封裝完成後才判定成品良否的 ATE final test。
...
- **錢包占比**：客戶全部設備支出中，分給某類工具的比例；工程更重要不等於占比已上升。

### 三句話抓重點

- 三家設備商的一手文件分別支持需求方向、缺陷機制與客戶資格節點。
- 核心問題是複雜度是否真的增加每代產品所需的量測、檢查與良率學習步驟。
- 現有資料不證明全產業錢包占比或台灣公司訂單，目前只做族群觀察。

### 為什麼重要

先進封裝把多顆昂貴晶片綁在一起，越晚發現缺陷，報廢代價越大，價值可能因而前移到製程中的量測與缺陷回饋。但新工具或資格認證只證明工程問題與採用節點；產業獲利池是否擴大，仍要看工具數、採購與收入組成。

### 接下來怎麼追

- 追設備商是否揭露有期間與定義的訂單、收入、工具數或量產採用。
- 追 MOPS 是否出現台灣公司的具名產品、客戶階段與財務貢獻。
