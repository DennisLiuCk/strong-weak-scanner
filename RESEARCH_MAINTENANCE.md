...
2. 先用交易所／MOPS 與官方法說日曆，對**全 universe** 做前次掃描窗口以來的事件
   ...
   - TWSE 與 TPEx 的重大訊息端點，以及當季損益表／資產負債表 OpenAPI；兩市場與完整
     ...
     **另要求每個市場均實際觀測到請求窗口的每個日曆日**，不能只命中其中一天；重大訊息日端點
     只保留單一出表日期批次（實測全部列的發言日期僅有出表日期減一那一天），批次滾過
     窗口後舊發言日永久消失，只靠日期算術會把「批次完全不含窗內任何一天」誤標成
     `full`。批次已滾過窗口時一律維持 `partial`，缺口只能沿用當時真的讀到該日的既有
     scan row，不得由後來的重跑追認。
     缺日（含週末）也維持 `partial`；缺日是未知，不是零公告。輸出的
     `missingSpeechDatesByMarket` 逐市場列出缺日，不得拿兩市場日期聯集互補。
     既有 scan row／方法快照不改寫；沒有保存逐日證據的歷史 full 不能作今日完整覆蓋的證明。
     任一市場
...
改寫任何 block。同節確定性命中 1–3 個時節首直接顯示全部詞鈕；命中 4 個以上時以預設收合、
可由鍵盤操作的原生 `details` 明示名詞先不用背，讓正文先出現。展開後仍須一次保留全部命中詞、
完整詞名、同一 dialog、原字典順序與原始定義，不得因漸進揭露刪詞、重新排序、挑選重要詞、
另建第二層剩餘詞清單或另寫解釋。
...
已通過系統位置條件的 row 卡在窄欄不得繼續用「窄標籤欄＋窄內容欄」壓縮句子；每個 cell 改成
全寬內容段，前方依原欄序顯示 `01 · 先定位`、中間欄 `NN · 接著看`、最後欄
`NN · 最後核對`，並在同一提示列逐字顯示該欄原 `th`。這些固定 cue 只安排閱讀先後，須對輔具
隱藏，讓原生 table header 繼續提供欄位語意；第一欄另保留可被輔具讀到的實際
`位置 N／總數` 文字節點。寬欄原生 table、一般 row 卡、原 `th`、原 cell、row 順序及研究 payload
皆不得改變。

通過系統位置條件的 table 若後方**緊接**一個純文字 paragraph，且正規化後能依原字序唯一切成
「前置原則 → `這張表只…` → `它不能…`」三個非空片段，reader mode 可用
`readerSystemTableTakeaway()` 取代該 paragraph 的一般排版，顯示「先記住／能說到這裡／先不能說」
三句收束卡。切分只能用兩個作者明寫的固定起點，不得讀 table cell、題名、族群、route 或模型摘要
判定結論；任一片段超過 220 字、前置原則不以句號結束、原段含連結／粗體、不是緊接系統表，或
任一條件不唯一時都安全退回原 paragraph。三張卡的文字依序串接後須逐字等於同一個正規化原段，
原 Markdown、runs、block 順序與 payload 不得回寫；標籤只安排讀後收束，不得新增研究結論。
寬欄可三欄並排，section 容器不超過 620px 時須單欄，且不可新增互動控制或鍵盤停駐點。
...
正文節名下方的本節名詞只比對該節既有標題與 blocks，並逐字連回同篇名詞小字典。命中 1–3 個時
直接顯示全部詞鈕；命中 4 個以上時必須用預設收合的原生 `details／summary` 顯示「名詞先不用
背」，summary 明示先讀內容、遇到陌生詞再查，手機至少 56px 高。展開後須一次列出全部命中詞，
順序仍沿用原字典；不得刪詞、分出模型挑選的「重要詞」、改寫定義或另建第二層剩餘詞清單。
收合時詞鈕不得進入鍵盤 tab order；展開、開啟原字典對話框與關閉後返回原詞都不得改變文章 hash
或正文捲動位置。
...
並同步顯示當前路線、主題、視角、證據層級與可選的台股範圍；展開後 summary 仍須可見，
讓滑鼠與鍵盤都能再次收合，不能形成只能打開、不能關閉的單向控制。桌機四步讀法預設展開；窄幅
首次載入才把四步讀法也收合一次。兩個區塊都不得在旋轉或 resize 後覆寫讀者手動開合狀態。
控制摘要之後必須固定顯示一張動態「目前讀圖任務」：標題逐字取正式 route `question`，第一格逐字
取當前 phase 的 `purpose`，第二格逐字取當前 graph `summary`；路線／中心主題切換後須同步更新
route、phase 與 graph ID 的可機讀定位，以及三層可見文字。圖譜主介紹不得再次重複同一份 graph
摘要，只保留如何讀關係與財務歸因的共通說明。這張卡只整理正式課綱與既有圖譜摘要，不得依圖上
公司、edge 數量、標題、正文、相似度或模型另寫系統問題、階段任務、供應鏈、受惠或投資排序；
窄幅須改成單欄且不得因控制收合而消失。
