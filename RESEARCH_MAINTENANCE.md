...
受惠方向或已證實公司連結。發布卡正面顯示這五項，族群問句按鈕負責定位族群矩陣的起讀文章
...
改寫任何 block。市場議題一般正文內的 table 另有窄欄閱讀契約：每個 `th` 必須保留原文並以
...
成熟度／證據邊界藏成預設不可見資訊。標題與查核警語後另顯示一張
「新手閱讀任務」：`build_dashboard.py` 只擷取「為什麼重要」開頭的完整句子與「想一想」
第一題，分別標成「這篇先釐清」與「讀完試著回答」；不得自行重寫、摘要或補入新主張。
所有被登錄為學習路線主文章的文章都必須能產生這張卡，缺任一來源段落時 dashboard build
直接失敗，不能讓族群起點退回只有術語與查核資訊的首屏。
閱讀任務的主行動固定標成「開始讀三句重點」，只可定位同篇「新手先讀」內既有的
「三句話抓重點」，不得跳過、改寫或另造摘要。定位時須同步移動鍵盤焦點；手機讓 heading
停在黏性導覽下方約 120px，桌機則在 `readerScroll` 頂端下方約 76px，且不得改變文章 hash。
正文後「回看本篇三句重點」必須共用同一個定位函式，避免兩個入口日後產生不同焦點與 offset。
產業角色與路線資訊仍保留在閱讀任務下方，主行動旁須明示讀者可繼續往下比較族群角色。
...
發布頁另由 v3 register 自動合成「研究摘要：已知、未知與下一步」；頁首閱讀任務後，完整
「新手先讀：這篇在講什麼」再緊接研究摘要，讓不熟悉產業的讀者先取得主線、名詞、角色與判讀
...
由族群矩陣開文且原族群確實在本文範圍內時，預設先顯示該族群；其餘入口顯示本文第一個正式
族群。多族群以 `aria-pressed` 單選切換並用 `aria-live` 更新原文指南，超過四個族群時預設收合
完整選項，避免事件錨點在手機首段鋪滿十一個按鈕；沒有正式族群的文章不顯示這個區塊。並列只
代表本文宣告的研究範圍，不得改寫成上下游、受惠、訂單或投資排序。

其後的「先選一個系統問題」必須直接使用 `RESEARCH_LEARNING_ROUTES` 的 label、
...
而沒有清單位置時，則把目前文章卡捲入可視區後聚焦，不能把焦點留在已隱藏／已移除的返回按鈕。
...
堆在內容之前。窄幅首次載入時，四步讀法與完整控制各自收進原生 `details`；前者摘要保留用途與
非排名邊界，後者摘要必須同步顯示當前路線、主題、視角、證據層級與可選的台股範圍。完整內容在
無 JavaScript 的預設 markup 仍為展開，手機只在載入時收起一次，不得在旋轉或 resize 後覆寫讀者
手動開合狀態。每條路線的說明必須交代建議閱讀順序；每張 active graph 至少要有一個既存
...
語氣；下一站問題逐字取自下一篇已登錄 route 主文章的閱讀任務。任何 route 主文章缺少問題或
