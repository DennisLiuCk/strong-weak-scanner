...
閱讀任務的主行動依來源標成「開始讀三句重點／開始讀 30 秒摘要／開始讀多空觀點」；市場議題
定位同篇「新手先讀」的「三句話抓重點」，正式筆記定位「30 秒摘要」，多空小作文先定位
「多空觀點（小作文）」而不跳過兩邊敘事。定位時須同步移動鍵盤焦點；手機讓 heading
停在黏性導覽下方約 120px，桌機則在 `readerScroll` 頂端下方約 76px，且不得改變文章 hash。
正文後「回看本篇三句重點／30 秒摘要／勝負手」必須共用同一個定位函式；多空文章回看時定位
「勝負手」，其起讀行動仍定位整個多空段落。市場議題 DOM 閱讀順序固定為「閱讀任務 →
...
族群矩陣在桌機必須同時顯示「先認識一個族群」與「先選一個想弄懂的問題」；在 780px 以下則顯示
兩個至少 56px 高的入口按鈕，預設選取「從問題開始」，讓不熟族群名稱的讀者不必先穿過 11 個術語。
...
其後的「先選一個想弄懂的問題」必須直接使用 `RESEARCH_LEARNING_ROUTES` 的 label、
...
站點按鈕的顯示層必須先以 `station.question` 作為主標，再用「研究節點：」標示既有
`station.graphLabel` 為次要脈絡；不得讓技術節點名壓過讀者要回答的問題。按鈕最小高度 72px，
`aria-label` 必須同時包含問題與研究節點。站點按鈕直接開啟該文並回到頁首，
