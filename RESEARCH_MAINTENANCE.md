...
任務卡的 DOM 與視覺順序必須先放標題、兩個任務句與主行動，再放可選的符號解碼、名詞說明與
「為什麼值得讀」，最後才顯示來源邊界；不能讓輔助說明把「開始讀三句重點／先看產業角色」推到
首屏之外。順序調整只作用於 renderer，不得改動 `readingMission`、原句、來源或 action target。
...
直接圖譜 deep link、頁首圖譜分頁、研究雷達與非文章入口必須清空此狀態；同樣不得寫進 payload。
