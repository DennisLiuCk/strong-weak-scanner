...
至少兩個 `。！？`，可在句末插入 `aria-hidden` 的零文字停頓；只要實際 `.article-section` 容器
不超過 860px 就須顯示，不能只以整體 viewport 判斷。`≤1180px` viewport 用 0.55em，較寬視窗中
仍不超過 860px 的專注閱讀欄用 0.45em；段落仍須維持同一個 `<p>`、原字順序與可複製全文。
門檻是版面契約，不是內容品質分數，也不得據此生成摘要或改寫句子。查核附錄、研究摘要與新手
三句重點不套用長段落斷句，避免改變證據表與既有卡片結構。
...
`.maturity-route-card` 必須跨滿 `.maturity-route-cards` 整列，讓階段以至少 240px 的自適應欄寬
並排，單階段站點則以至少 280px 的自適應欄寬呈現。780px 以下維持單欄 DOM 閱讀順序，不得為
了桌機比較而橫排。這只調整既有 route、phase 與 station 的排版；標籤、問句、站次、文章映射、
開合語意都不得另建或改寫。路線 map 的原生 summary 必須可用 Enter 或 Space 明確切換，焦點留在
同一 summary；不得把鍵盤使用者迫使到第一站按鈕才能查看路線。
...
反向從文章「看產業關聯」卡進圖時，顯示層另建立一次性的 article-learning graph origin，保存
文章 ID、文章閱讀欄與 window 捲動位置，以及文章原本的矩陣／圖譜起點。圖譜首屏必須先說明
「文章 → 關係圖」、剛才文章與返回後可繼續的站次，並把鍵盤焦點送到這條脈絡；返回後須恢復
同一篇的「從這篇接著學」位置、原圖譜按鈕焦點與原本系統問題，不能回文章頂端或遺失矩陣起點。
