...
- **`CLAUDE.md` 與 `AGENTS.md` 內容必須完全相同**——改任一檔就同步另一檔,改完
  `diff CLAUDE.md AGENTS.md` 確認為空(兩檔曾被不同 session 各自更新而分岔,
  導致後續 agent 讀到的入口文件缺關鍵規範)。
- 所有會寫回 `main` 的 Actions workflow 必須共用 `repo-main-writer` concurrency group、
  `queue: max`、實際開始時 checkout 最新 `main`；push/rebase 衝突必須標紅,不得 `exit 0`。
  Daily complete push 後必須確認 Pages latest build 已部署同一 commit。
...
validate.py      → reports/ 週報(§⑥=觀察因子 IC、§⑦=元素邊際貢獻、§⑧=tier 持續性、
                 ...
                 判讀門檻**不是 1.96**(MC 顯示那樣誤判率 10~24%),依有效獨立觀測分級
                 (<6→4.0、<12→3.0、<30→2.5、否則 2.4)。套上分級後全表只剩
                 s_resil 全期(t=−3.1)可分辨。
