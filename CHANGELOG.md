...
系統原本只在一個座標上說話：**族群內相對**、**20 日**。兩個軸從來沒出現在畫面上，
而它們一缺席，讀者就只能把相對排名讀成漲跌。
...
- **絕對報酬軸**。2026-07-24：相對強勢（真強／蓄勢·外資佈局／強但過熱）23 檔裡
  ...
  個股列在綜合分右側並列絕對報酬，上層 tier 但絕對為負者加 ↓；族群卡加
  「絕對 −10.4% · 1/10 檔漲」。導言檔數由當日資料現算，0 檔時整句消失。
- **時間尺度軸**。同一檔在短期（close/ma5）、波段（ret20）、趨勢（ma20/ma60）
  三個尺度各排一次族群內名次。冗餘門檻沿用分歧視角的 |ρ| ≥ 0.8；41 個交易日
  （2026-05-27 起，ma60 需 60 日暖身）逐日 ρ 中位為 短↔趨 +0.04、短↔波 +0.32、
  波↔趨 +0.49，三者互補；ρ(ret20, rs20) = +1.00 印證波段欄就是現行視角。
  最初的趨勢定義 `close_adj/ma60` 對波段 +0.78、對 `ma20/ma60` +0.79，逼近門檻且
  兩邊都像，已否決並以測試釘住不得復活。今日三欄極差中位 46 個百分位點，58/121 檔
  達 50 點以上——例:南電 tier=真強、波段與趨勢皆 100 分位而短期僅 9（剛開始回檔）；
  健策波段 100 而趨勢 0（剛從長期低檔翻上來）。
- ρ 逐日算再取中位，不取單日：2026-07-24 的短↔趨單日是 −0.29，全期中位 +0.04。
  窗口只有 41 天且幾乎全在同一段回檔裡，ρ 在其他行情下未必相同；此處只當冗餘篩，
  不當假設檢定，故不附標準誤——這點在畫面上直接寫明。
- 新增 21 條契約測試（`test_absolute_return_contract.py` 10 條、
  `test_time_lens_contract.py` 11 條），每條的核心斷言都做過突變檢查，破壞後確實
  失敗。釘住三個實作時真的踩到的坑：payload 漏欄、`D.allStocks` 投影欄名與 payload
  不一致導致警語靜默空掉、`toFixed(0)` 把 −0.4% 印成「-0%」。全套 277 tests OK。
