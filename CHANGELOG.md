...
## 研究循環：續刊、期限補漏與來源邊界 — 2026-09-18

- 安全接續9/13未發布研究，保留原查核日期；更新8篇既有文章，不重複建文。補入
  Rubin多櫃bring-up、High-NA累計量分母及PFAS申報起點更正；其餘涵蓋OFAC授權、
  同欣電Q2指定頁、ERCOT補件、PCIe工作坊世代與OPG-M頁碼。
- 本日39個原到期monitor皆回查，另列1個接續項，共40筆（2支持、1反證更正、
  9無新證據、28未可測）；加上9/13的21筆，本次新增61筆append-only紀錄。
  數字為帳本盤點，不是獨立樣本或方法支持率；舊主命題時鐘、DB及archive不改。
- 雷達保留SSCB、HBFSim、玻璃核心三題watch；新手段落與產業圖譜同步更新，沒有
  把資格、累計處理量或制度鄰接升格為台廠財務受惠。方法增加監測機關路由與政策
  日期條件核對，新增具名回歸及方法快照；首頁與研究中心同步重建。
- Windows 11／系統Python3.12.10預設及PYTHONUTF8=1兩種環境，各699項測試
  （各跳過1項）、六lint／歷史檢查及隔離重建通過，保護檔雜湊未變。來源取得與
  視覺驗收限制詳見`notes/research_method_reviews/2026-09-18_review.md`。

## 機構情報第二輪：交付案例雷達與來源方法 — 2026-09-10

- 三篇案例各自提交後，將分期交付、電力設備轉單及材料許可放入現行研究雷達；
  前一輪文章增量及雷達歷史保留，材料題如實記錄由watch取得證據後升格。
- 來源方法增加J.P. Morgan交易角色與MSCI模型邊界，整理兩輪試讀及人工回查入口；
  沒有建立排程。研究與逐篇發布驗收見第二輪來源報告。

## 機構情報第二輪：鎵與基板出口交付案例 — 2026-09-10

- 以MSCI7/16研究對照AXT季報及公司稿，分清原料提純、砷化鎵／磷化銦、目的地
  許可與實際交付；保留亞洲歐洲獲批反方及待驗證收入影響，未外推台廠受惠。
- 新增四條關係圖、中文問題與政策閱讀路線。原先日期不足的S&P草稿不入帳，
  選題watch歷史保留；五份來源封存與離線核對詳見第二輪報告。

## 機構情報第二輪：燃氣設備預約轉單案例 — 2026-09-10

- 新增S&P MI背景與GE Vernova公司文件對讀文章，分列預約、訂單、出貨及美元履約義務；
  保留施工準備、預付款節奏及季度產出計畫的核對點，避免把合計容量外推公司收入。
- 加入五條關係圖、中文問題入口及資本投入閱讀路線。來源日期、原PDF、22檔離線
  雜湊核查與稿件邊界記錄於第二輪來源報告，每篇分開發布。

## 機構情報第二輪：Beacon Point分期交付案例 — 2026-09-10

- 新增短篇市場研究及四條關係圖，按期別拆開簽租、融資、通電與資料廳交付；
  J.P. Morgan的承銷參與角色、公司預期與未證實供應商證據均明示。
- 加入中文問題入口及資本投入閱讀路線，舊內容與時鐘保留。導覽測試改為保留歷史
  下限及原骨架順序，全部現行文章／圖譜的精確覆蓋檢查仍有效。
- macOS26.6.2／預設Python3.11.11下，692項測試、六lint、隔離重建與保護檔雜湊
  通過；Chrome桌機與手機閱讀檢查、離線來源複核見第二輪來源報告。

## 機構情報第二輪：分期交付與設備訂單選題 — 2026-09-10

- 研究前凍結 `RS-2026-09-10-02`：依序研究 Beacon Point 園區分期交付、GE Vernova
  產能預約轉訂單與出貨；鎵／基板出口許可因機構草稿發布日缺口先列觀察。
- 每題保留第一拒絕與下一份證據，後續按研究結果決定新文章或併入既有內容。
  選题僅安排人工工作，不是股票排名；本輪起始基準為 `6965507`。

## 機構情報試作：三題雷達與擴充結果驗收 — 2026-09-10

- 依已凍結順序將場址、AI融資、記憶體成本三題放入新active radar，連回各篇9/10
  更新；保留舊雷達歷史、原主命題時鐘及圖譜邊界。來源方法納入研究維護入口。
- 修正擴充既有文章被算成研究後拒絕；新增獨立結果計數，watch/defer轉向亦如實記錄。
  雷達與矩陣的文章入口依狀態區分新升格及既有文章增量，舊snapshot不改寫。
- macOS26.6.2／預設Python3.11.11、Node22.14.0：692項測試及prepublish通過，
  DB/archive雜湊不變；Chrome1440/390/320px實開三篇新段落並返回雷達，無JS錯誤或
  水平溢出。三篇內容已各自commit/push，CI與Pages完成，明細見本輪來源檢查報告。

## 機構情報試作：記憶體成本與買方分化 — 2026-09-10

- 將Morgan Stanley採購分層、S&P MI替代風險與HP最新10-Q及財報稿接入記憶體文章；
  分開營收、PC台數、平均售價與毛利率，保留供應改善反方及單一買方邊界。
  新S24–S27、C31–C35、T8；來源日期、申報期間、獨立複核與SHA留在本輪檢查報告。
- macOS26.6.2／預設Python3.11.11：685項測試與prepublish通過，DB/archive雜湊不變；
  Chrome1440/390/320px全文無水平溢出及JS錯誤，目視桌機和手機新內容。

## 機構情報試作：AI融資展望與公司指引對帳 — 2026-09-10

- 將S&P Ratings 8/27公開研究發布稿與Microsoft7/29法說接到現金回收文章，
  保留FOCF/FCF、年度月份、租賃分類及預測版本的差異，加入反方與季度回查入口。
  新S23/S24、C28–C31、T6不刷新C2主命題；來源定位及SHA留在本輪檢查報告。
- macOS26.6.2／預設Python3.11.11：685項測試、六項lint、隔離build及保護檔雜湊檢查
  通過；Chrome1440／390／320px無JS錯誤及水平溢出，目視桌機與手機新段落。

## 閱讀體驗續查：公司文章標題先顯示公司與文類 — 2026-09-10

- P2：正式筆記與多空小作文原先把通用讀後問題當大標題，同一句又出現在閱讀任務中。
  改以原研究頁名為主標題，閱讀任務保留完整問句；市場議題仍以具體研究問題起讀。
  內容、查核狀態、原始來源與段落導覽皆保留。
- macOS 26.6.2／預設 Python 3.11.11、Node 22.14.0：257 項研究相關測試通過；
  行為測試涵蓋正式筆記、小作文、市場議題與沒有問句的頁面，並檢查標題焦點契約。
  in-app browser 320／390／1440px 檢查標題排版、完整問句與「先看產業角色」跳轉。

## 機構情報試作：場址供電與液冷取捨 — 2026-09-10

- 新增機構取材方法並在深研前獨立提交三個研究問題；本段把高盛9/1公開逐字稿與
  Microsoft Pecos 6/22原始計畫接到液冷文章，分開機構分析、客戶規劃與場址實績。
- 增加S25–S27、C32–C34及10/10人工查核T9；原公司財務主命題、可信度與舊期限保留。
  SHA、直接HTTP與web文字截取差別、日期及推論邊界留在本輪來源檢查報告。
- macOS26.6.2／預設Python3.11.11：684項測試與prepublish通過，DB／archive雜湊不變；
  Chrome 1440／390／320px全文可讀且無水平溢出。內容測試保留歷史下限與逐項斷言，
  允許來源、主張和monitor追加，歷史不可改寫仍由baseline閘門檢查。

## 閱讀體驗續查：手機研究篩選完整操作循環 — 2026-09-10

- P2：篩選面板開啟時，背景改為 inert 並鎖定捲動；Tab／Shift+Tab 留在面板內，
  正確處理同組 radio 只有目前選項進入 Tab 順序。補上遮罩、固定關閉列及完成篩選按鈕。
  完成、Escape 或點遮罩均保留篩選並還原焦點；切換桌機／手機斷點會還原背景狀態。
- macOS 26.6.2／預設 Python 3.11.11、Node 22.14.0：256 項研究相關測試通過；
  新增 radio 焦點順序回歸案例。in-app browser 390×844 實測雙向 Tab、三種關閉方式、
  記憶體篩選後仍顯示 33 篇，及 1440px／390px 斷點切換。

## 閱讀體驗續查：個股明細保留關閉與段落導覽 — 2026-09-10

- P2：個股與族群抽屜新增固定標題列，閱讀下方內容時仍看得到名稱與 44px 關閉按鈕。
  手機的七顆跳段按鈕改為完整名稱選單；上一／下一檔並排，段落跳轉避開固定標題列，
  並將鍵盤焦點送到目標標題。減少動態偏好下不使用平滑捲動。
- macOS 26.6.2／預設 Python 3.11.11、Node 22.14.0：51 項 dashboard 測試通過；
  in-app browser 390×844 深淺主題實測跳轉股價圖、直接關閉、焦點回復與相鄰個股切換。
  原始資料、策略與既有歷史快照不變。

## 服務與 UX 審查：研究雷達顯示逾期狀態 — 2026-09-10

- P2：雷達的總檢查日期 2026-08-16 已過期，頁面仍固定寫「下次總檢查」。
  依既有台北日期時鐘區分已到期、今日到期、未來期限與未設定；保留原始期限，
  不刷新雷達選題、研究證據日期或可信度。
- macOS 26.6.2／預設 Python 3.11.11、Node 22.14.0：日期分支 JS 行為測試通過；
  in-app browser 390×844 確認「總檢查已到期 2026/08/16」已呈現。

## 服務與 UX 審查：恢復歷史快照入口 — 2026-09-10

- P2：README 有歷史日期選單說明，builder 也生成日期清單，但模板未使用它。
  快速前往新增歷史日期選擇與另開分頁連結，使用 build 時實際存在的 archive 日期；
  新版歷史頁另提供返回最新掃描。下拉導覽在低高度視口可捲動。
- macOS 26.6.2／預設 Python 3.11.11、Node 22.14.0：51 項 dashboard 測試通過；
  新增首頁／歷史相對路徑及不合法路徑行為案例。in-app browser 實點 2026-09-08，
  確認另開的既有快照顯示 9/8 資料日，原首頁保留。正式 DB 與 46 份舊快照無 diff。

## 服務與 UX 審查：Pages 查詢失敗仍保留健康報告 — 2026-09-10

- P2：`--check-pages` 在 GitHub CLI 不存在、查詢逾時或回應無法解析時，原本可能
  丟棄已完成的本機檢查而退出。遠端查詢獨立容錯並列明「未確認」與原因；
  只有 built 且 commit 與本機 HEAD 一致才列完整，errored 仍是發布失敗。
  GitHub CLI 固定從專案目錄執行，錯誤摘要不回顯原始回應或憑證。
- macOS 26.6.2／預設 Python 3.11.11：7 項 operational_health 測試通過。
  實際 build_health 對缺 CLI／逾時保留逐項相同的本機結果，並測過無效 JSON、
  錯誤格式、查詢失敗、建置中、舊 commit、部署失敗及同 commit 成功。
  本機受限網路執行亦成功輸出本機健康項目與 Pages 未確認提示。

## 服務與 UX 審查：串接族群、個股與分歧名單 — 2026-09-10

- P2：族群明細原本只有聚合數據、沒有成分股入口；兩視角分歧名單也無法點開。
  族群面板新增成分股按鈕，個股面板可返回來源族群，上一／下一檔保留族群上下文。
  分歧列補上鍵盤與點擊入口及 44px 目標；面板切換回到頂端。
- macOS 26.6.2／預設 Python 3.11.11：50 項 dashboard 測試通過。
  in-app browser 實測散熱→雙鴻→返回散熱、Escape 關閉回到原啟動按鈕、
  以 Enter 開啟分歧列緯穎；既有背景 inert 與焦點限制維持。

## 服務與 UX 審查：平板導覽列與窄幅控制項 — 2026-09-10

- P2：732px 視口的品牌最小寬、導航與工具列互擠，快速前往變直排、主題按鈕超出
  右緣。將 header 分列斷點獨立提高至 900px，工具列不壓縮、選單文字不換行，
  極窄手機品牌可省略，保留所有操作控制項。
- macOS 26.6.2／預設 Python 3.11.11：dashboard UX 契約測試通過；in-app browser
  732px、320px 的 document scrollWidth 均等於 viewport，並目視確認控制項可見。
  同時檢查桌面排列與快速前往、主題切換。此項僅調版面，不新增套件。

## 服務與 UX 審查：個股搜尋的範圍與空結果復原 — 2026-09-10

- P2：在預設被動元件搜尋「3260」原本只有空白表格與 0/12，沒有跨族群查找入口。
  新增全部族群選項、範圍提示、空結果說明、其他族群命中入口與清除搜尋；
  加入搜尋框標籤及結果 live region，忽略輸入前後空白。
- 全部模式按族群分組顯示，不跨族群排綜合分；搜尋保留該族群原排序位置，
  暫停／未交易仍置底且無名次，同值以股號固定順序。
- macOS 26.6.2／預設 Python 3.11.11、Node 22.14.0：50 項 dashboard 測試通過，
  新增行為案例覆蓋搜尋範圍、空白／大小寫、原位置、反向排序與暫停列。
  in-app browser 390×844 驗證「3260」空結果→全部族群→威剛，以及結果與分组提示。
...
- 修正 `research_event_scan` 多日窗口只命中一天即可 `full` 的漏洞。逐市場列出
  `missingSpeechDatesByMarket`，末日、兩端、跨市場聯集與缺少週末都不視為完整；
  `--require-full` 對缺日返回 2，JSON 固定 UTF-8/LF。保留舊 scan 與快照，不追認
  缺乏逐日存證的歷史 full。新增七項回歸案例及三項文章／圖譜契約測試。
- 11 個原到期 monitor 及 CPO 接續 T5 共追加 12 筆：2 `new_support`、1 `new_contrary`、
  9 `not_yet_testable`。這是工作事件分類，不是主命題支持率或投資命中率。
  CPO 的反方結果僅指原文不支持舊用語，不代表產品未生產或共存命題被否證。
- CPO 保留舊 C1/S1，新增 C24/S18 將 Spectrum-X 原文縮窄為 `now in production`；
  `full production` 主詞為 Vera Rubin。T3 退休、T5 保留相同觸發、失效及 8/26 期限，
  四條圖譜線改接 C24 且日期不變；新增新手主詞查核，不刷新 C9 主命題。
- 旺矽官方簡報與英文季報完成引用及相鄰頁目視、同檔 SHA 與獨立加總重算，補入
  測試需求文章 S22/S23、C15/C16 及 M4 不可比帳本。新增公司總額／未披露 AI 分子的
  財務分母線與 assessment，不由探針卡占比反算假精確收入、不推 AI 毛利或受惠。
  活動日、核閱日、首次上線未知及捕捉日分開，原主命題與舊時鐘不動。
- 同日凌晨／晚間複查抓到同欣電 9/9 14:48:06 新增英文 Q2 附件，實取 417,028 bytes，
  SHA `114904b9b32065b98a61d7ad223603ff49e172363e79c3d0727202967dca39c0`。
  Q2 文章 S35/C33 只更新附件里程碑；同欣電、德微仍待完整獨立複核，不重複簽核。
- Azure 區域表已讀到完整 VM 子列，未列 MI455X v7 不能判定未上線；公司附件未全核者
  如實待測。欣銓／SPIL 買方設備公告、其他擴產與月營收不升格 AI 財務；弘塑治理異動
  留下一次定向查核。OIF 展示 PDF 影像未核完，不接受新 claim，不為湊篇數新建文章。
- 原雷達追加定向路由而不重排凍結值；9/5 已做月初方法回溯，本輪另追加 9/9 方法快照。
  發布前保護基準為起始 `5dc2952cbafdebf25fd0c40cca2446100aac3d67`；同日續跑安全拉取
  Actions checkpoint `a2ec062`，研究不寫 DB、策略、IS_CUTOFF 或 OOS。
- 發布前在 Windows 11／Python 3.12.10 的預設編碼與 `PYTHONUTF8=1`（含子程序）各跑
  677 項完整測試：676 通過、1 項 Windows symlink 權限條件略過；研究 lint、歷史
  append-only 檢查及隔離 dashboard build 均通過，DB 與 45 日 archive 保護檢查通過。
  31 個 stale topic／566 條 stale edge 仍保留降級，不因追加 review 或編輯文章刷新期限。

## 研究循環：產品代別邊界與正式筆記進度同步 — 2026-09-07

- 19 個到期 monitor 均追加回查結果，另提前登錄 Helios T13；共 20 筆為 2 個
  `new_support`、8 個 `no_new_evidence`、10 個 `not_yet_testable`。公司附件語意查核
  及 Azure 動態狀態不完整者排 9/9 接續，不以查核不足冒充無新證據。
- Helios 文章新增 S25／C27／C28、新手案例及 T13，拆開 AMD／Cisco／HUMAIN 聯合稿
  中 MI355X 已運行與 MI400 後續部署計畫；一份聯合稿只計一條消息鏈。圖譜新增
  KG-HEL-I28 產品代別／品牌系統／場站／日期共同鍵，不新增 HUMAIN→Helios 客戶線、
  台灣收入或獨占線，C11 及原公司邊日期不動。
- 威剛 Q2 正式筆記已在 `a1de741` 獨立簽核，本輪只同步市場議題 impact 為 done、
  新手摘要與下一步，未重複研究或簽核；同欣電與德微仍待完整複核，C15 時鐘不動。
  增加兩項回歸契約，檢查完成狀態對應簽核筆記，以及兩篇更新進首頁但不洗新證據日期。
- 候選雷達追加定向回查說明，不另開凍結選題輪、不把局部查核當八題重新評估。
  方法文件刪除過時的支持率說法，明訂簽核／附件進度不等於主命題成立，並固定
  發布前比較基準為本輪起始 `1ddcd7fc0c2b0f540adead726964f270f481071f`。
- 9 月首輪方法快照已在 9/5 保存，本輪新增 append-only `2026-09-07_01.json`。
  808 個 active claims、859 條可回查圖譜邊；31 篇主題與 564 條舊邊過期仍維持降級。
  上述均為 registry／工作事件完整計數，非市場抽樣、支持率或投資績效，SE／t 不適用。
- Windows 11／系統 Python 3.12.10，在 `PYTHONUTF8=1` 與未設定該變數的預設編碼下，
  發布前八類檢查與隔離 build 均通過；各完成 667 項完整測試，666 通過，1 項 Windows
  symlink 權限 skip，正式 DB 與 43 份 archive SHA 未變。初次 sandbox 執行遭系統
  暫存目錄權限阻擋，改用正常本機權限重跑；未改策略、IS_CUTOFF 或 OOS。
...
- `daily_brief.py` 與每日 complete workflow 加入 `operational_health.py`，分別顯示
  原始表、首次正式訊號、A–D 覆蓋、TDCC 完整級距與研究狀態。研究過期保留降級警示，
  不因每日原始表完整而隱藏；月／季資料新期未到齊不冒充正式缺口。
- 最新檢查日期取 universe 價格、大盤與評分之最大值，較新的 checkpoint 不會被舊
  完整評分日掩蓋。正式發布驗訊號、族群、大盤與當日所需排名 spec；完整修正版不能
  補救首次正式快照缺列。假說報告品質／筆記 SHA 失配也會進 P0 待辦。
- `prepublish_check.py` 與品質 CI 共用筆記、假說、研究歷史、圖譜、雷達、方法帳本與
  完整測試，並以 archive 副本隔離重建及比對首頁／研究頁。無效或非祖先 Git 基準直接
  失敗，不退回空樹；正式 DB／archive 前後 SHA 納入驗收。
- Windows 11／Python 3.12.10，`utf8_mode=0`、未設 `PYTHONUTF8`：固定本輪起始基準
  `fd4a4ab` 的八類檢查通過，完整 665 tests 通過（1 項 Windows symlink 權限 skip），
  DB 與 43 份 archive 未變。健康普查為原始／正式訊號完整、A–D 各 121/121、
  TDCC 121/121；39 篇活躍議題中 31 篇證據過期，故整體仍為需處理。
  以上是當下完整性計數，非抽樣統計，SE／t 不適用；未宣稱新排名已有正式 OOS。

## 研究待辦合併與威剛 Q2 正式基準 — 2026-09-07

- 新增 `research_worklist.py`，同一公司／議題的原始警示合併成一件工作；P0 全列，
  其他先依原優先級，再依新季報、到期假說數及最早期限，預設只列五件。
  有效 monitor review 可安排純議題下次工作日，不刷新文章證據日期，也不延後品質或 impact action。
- 威剛以四份公司官方 PDF、28 個主張更新至 2026Q2。不同 reviewer 對同一 pack 離線
  核對 SHA、目視全部 28 個引用頁及九個相鄰頁，重算 50 項算式／單位；單篇筆記與
  manifest 已獨立提交為 `a1de741`，原始 PDF／PNG 留在 `tmp/`。
  H1／H2 報告同步對照新的筆記 SHA 與財務基準，但終態複核仍未完成；保留原 8/31
  期限、open／weak、原始來源鏈與 transition，不把更新筆記視為假說成立。
- 9/7 工作量普查：599 條警示、162 件合併工作，三件純議題依既有 review 等待下次檢查。
  這是全集計數與工作覆蓋，不是抽樣估計或獨立觀測，SE／t 不適用；沒有使用頻率資料加權。
- Windows 11／系統 Python 3.12.10 預設編碼：完整 662 tests 通過，1 項 Windows
  symlink 權限 skip；筆記與領先假說品質契約通過。正式策略、權重、tier、OOS 不變。

## 首頁與週報共用 OOS 證據狀態 — 2026-09-07

- `evidence_status.py` 統一首次正式發布、交易日成熟度、有效獨立觀測與連續區段。
  首頁不再以成熟 10 日宣稱可判讀；達最低樣本數也只提示逐指標檢查 §⑨ 與 §⑩。
  以 9/4 資料核對為 38 個正式日、28 個成熟日、約 2.8 個有效獨立觀測及 3 段；
  低於估計 SE 的最低門檻 3，SE／t 不可估，未調權重、tier 或 `IS_CUTOFF`。
- 首頁排除資料日之後及非標準檔名的報告，明示報告日期；每週 workflow 在產報後
  同步重建並檢查首頁／研究中心，避免週報已存在但連結仍落後一週。
- 修正週報 console 摘要的 med_dip 基準變數被 §⑦ 字典覆蓋的錯誤。
- Windows 11／Python 3.12.10 預設環境：655 tests 通過，1 項 Windows symlink
  權限 skip；瀏覽器確認樣本限制與 9/4 報告連結，console 無錯誤。獨立 SQL／交易日
  切片重算與首頁逐項一致；DB、43 份 archive 雜湊未變。

## D 基本面採共同到齊期間，早報資料分開提示 — 2026-09-07

- 9/4 原始快照的 D 覆蓋為 16/121；8 月營收只到 16 檔就帶動全 universe 切期。
  改成正式 universe 當期原始值 100% 到齊才切換月／季，參考股票不參與選期；
  提前公布的期間顯示覆蓋與缺口，共同比較期間、必要 component 缺值各自保留。
- ranking schema 2／新 spec 自 9/7 登錄，舊新 spec 不串接；Champion 權重、tier、
  `IS_CUTOFF` 與既有正式快照不改。下一次正式日更才開始累積新 spec，部署前資料日
  的首頁是現行規則重算，不能冒充新的正式 OOS。
- 首次可見時間取所有實際使用的月／季比較資料之最大值，避免歷史補援輸入比顯示時間晚。
  新增早報者、同批到齊、first-seen 時差、參考股隔離、缺母體、缺比較期與季報欄位測試。
- Windows 11／Python 3.12.10，未設 UTF-8 環境覆寫：651 tests 通過，1 項因 Windows
  無 symlink 建立權限跳過；hardlink／正式路徑護欄仍測試，Linux CI 保留 symlink 測試。
  正式 DB 唯讀重算 D 為 121/121（全母體覆蓋計數，非抽樣估計，SE 不適用），
  第二條 SQL 路徑確認共同月份 2026-07、8 月已收 16/121；A/B/C、Champion 與兩個
  shadow 的 121 檔結果逐檔不變。DB 與 43 份 archive 的 SHA-256 保持不變。

## 研究循環：光寶／DCX 液冷整合與雙時鐘顯示 — 2026-09-05

**正式策略、tier、權重、`IS_CUTOFF`、資料庫與歷史儀表板快照不變。**

- 以證交所重大訊息與光寶公司公告補入 2026-09-03 對 DCX 的 1.76 億美元擬議投資；
  交易安排、約 25% 交割後持股與合作範圍列為證實，策略整合路徑列為推論。交割、
  共同產品鍵、具名客戶驗收、出貨及財務貢獻仍待驗證，不把供應鏈鄰接改寫成營收受惠。
- 液冷文章新增新手可讀的策略投資／交割詞彙、五層證據表與追蹤問題；知識圖譜新增
  光寶到液冷的 `integration` 邊，材料性維持 `adjacent`，未建立財務歸因。
- 回查 28 個到期 monitor 與 1 個提前檢查：1 筆取得新支持、28 筆沒有合格新證據；
  後者只追加 review event，不刷新 evidence clock。另新增本輪 partial scan 與月度方法
  snapshot；事件數是帳本紀錄，不是獨立統計樣本，不計支持率、SE 或 t 值。
- 研究頁與首頁把「內容更新日」改讀 append-only transition 的最晚日期，主命題可信度
  仍讀 claim ledger 的 `last_evidence_at`。因此方法補強與支線新證據會出現在最新文章，
  但不會為過期或未獲新證據的命題重設時鐘；新增回歸測試防止兩種日期再次混用。
...
- 智原正式筆記以年報、年度查核財報、Q2 核閱季報與 7/28 法說完成 `focused_v1`，
  32 個重要主張經不同 reviewer 使用同一凍結證據包離線複核；只將筆記與 manifest
  做成獨立 commit，PDF／PNG 留在 `tmp/`。保留稅費、比較期淨利、前季毛利與年度
  產品占比的來源差異；修正關係人分類及現金流／存量措辭，不猜匿名客戶或正常化 EPS。
- 獨立複核涵蓋 40 個引用頁、69 張圖與 125 項自行重算，root 另核對內容 SHA、
  全部 PDF／PNG 指紋、品質與引用頁完整度。這些是工作覆蓋計數，不是獨立統計樣本，
  SE／t 不適用；只關閉智原正式 Q2 待辦，其餘四檔維持待辦。
- 智原 H2 的原 OR 反證條件含「AI 占 MP 比例未回升」；公司 Q2 新聞稿同表列
  Q1 0.7%、Q2 0.4%，本輪依原條件進入 `refuted`，不推論指定 14nm 專案取消、
  未投片或未來不會量產。這是單一發行人兩季披露比較，SE／t 不適用；若各占比
  按最近 0.1pp 顯示，差額取位誤差上限約 0.10pp，仍不改下降方向。
- H2 原誤連的 Q4 新聞稿保留供追溯，補正 Q1 設計案來源；保留 7/12 首次存取、
  回溯收錄及初始 transition。Q2 另登錄 C2，不把同公司兩季文件當外部交叉驗證。
  H1 補到同場官方原稿，只升格公告證據，仍待驗且最晚複核日維持 8/31。
- 研究議題 C7／C17 的一位小數精度誤歸簡報，保留舊 claim，由 C30／C31 接續，
  明示只有新聞稿表格支持精確小數；既有代理值及條件區間以 Decimal 和另一研究者
  的精確分數重算一致，不是新 AI 營收或同批專案轉化證據。T10 因歷史 claim 引用
  由 T11 接續，原指標、來源、頻率、觸發／失效條件及原 next_check 逐字保留，
  C15 主命題與 source 時鐘不刷新。
- H# 描述性背景更新至 8/28，使用修正後還原價與融資口徑；另以 `db_ro`、
  獨立 SQL／Decimal 核對，DB SHA 不變。來源與背景核對環境為 macOS 26.5.2
  arm64、Python 3.11.11、預設 UTF-8；未覆寫 Python 編碼環境變數。
- 延伸既有回歸測試，核對來源更正鏈、T10／T11 原條件不變，以及 H2 終態仍保存
  初始期限與窄義反證邊界；導讀同步為剩餘四檔，不改寫歷史進度。
- 上述本機環境全套 620 項測試通過；品質、H#、議題、方法、圖譜、雷達與資料稽核
  均無硬錯誤。保留 23 項既有議題警告、原始法人表 74 筆 grid 外歷史列警告，
  以及 D 視角 117／121 的覆蓋警告；不把警告消失當作研究完成。
- 連續兩次建置的首頁與研究頁 SHA 相同，DB 與所有既有 archive SHA 未變；
  公開 HTML 隔離預覽在內建瀏覽器 1280×720 實查正式筆記、H#、Q2 導讀與首頁，
  無頁面橫向溢出或 console warn／error，原主命題日期與過期警示仍保留。

## 修正領先假說量化背景的價格與融資口徑 — 2026-08-29

**只修改背景產生器，不改計分、tier、OOS 規格、`IS_CUTOFF` 或 DB。**

- `leading_hypotheses.py --context` 的 60 日報酬改用 `close_adj`，與 20 日還原價報酬
  同口徑；原始收盤僅保留於收盤欄，還原價缺值沿用資料層的原價 fallback。
- `margin_chg20` 是餘額比值減一，改乘 100 並標百分比；例如 -0.10 正確為 -10%，
  不再誤標 -0.10pp。融資／借券水位明示占股本，與外資持股、借券水位的 pp 差額分開。
- 延伸既有 context 測試，涵蓋除權息口徑、融資百分比、缺值與原價 fallback；兩項
  context 測試在 macOS 26.5.2 arm64、Python 3.11.11、預設 UTF-8 通過。
  智原 8/28 背景另以 `db_ro`、獨立 SQL／Decimal 核對 14 個顯示片段，DB SHA 未變。
  這是單檔、4 個月份、61 個價格日與 9 個族群同儕的描述性核對，不是獨立績效樣本，
  SE／t 不適用。既存筆記快照未批次改寫，逐篇研究更新時採新口徑。
...
- 弘塑正式筆記更新為 `focused_v1`，使用修訂年報、更正年度查核財報、Q2 核閱季報及
  8/25 法說共四份核心 PDF；32 個重要主張由不同 reviewer 使用同一凍結證據包離線
  重算，來源 PDF／引用頁圖留在 `tmp/`，版本庫只保存筆記與 manifest。
- 明示年報產品標籤、匿名客戶占比，以及簡報 EPS／比較期業外符號的來源差異；分開
  核閱與查核、季度與半年、合約負債期初、借款／可轉債、期末與加權股數。不推估
  正常化 EPS、不對匿名客戶歸名，也不把 HBM／CPO 應用頁當量產收入。
- 弘塑 H1／H2 與原 8/31 檢查期限維持待驗；全年營收目標只能做進度檢查。描述性
  量化背景更新至 8/27，另以 `db_ro`、獨立 SQL／Decimal 核對，DB SHA 未變。
- 研究議題追加 8/25 場次與附件定位記錄，只關閉弘塑正式 Q2 筆記的待辦；其餘五檔
  完整複核與 C15 主命題時鐘保留。HBM 圖譜改指新筆記 S4，仍為應用機會；監測、
  掃描與方法稽核採追加紀錄，不重寫歷史。
- 離線複核的 35 個引用頁、79 張圖與 106 個算式是工作覆蓋計數，不是獨立樣本；
  本研究為單一發行人歷史財報核對，SE／t 不適用。執行環境為 macOS 26.5.2 arm64、
  pyenv Python 3.11.11、預設 UTF-8，未覆寫 Python 編碼環境變數。
- 同環境 608 項測試、研究各層 lint 與 append-only baseline 檢查通過；兩次網頁重建
  SHA-256 一致，DB 與全部既有 archive 的 SHA-256 前後不變。預設 1280px 瀏覽器
  實查正式筆記、假說與市場議題，保留原到期狀態，無水平溢位或 console 警告／錯誤。
...
**策略權重、tier 條件、regime 門檻、`IS_CUTOFF`、研究 source／claim／confidence／monitor、
正式 route／graph／edge、公司曝險、雷達候選排序與研究判斷零變動**；本次只替一般市場議題增加
一個 reader-only 正文捷徑，並把國巨 Q2 文章的新手開場改成正常中文文章語氣；同步研究維護契約、
方法清單、README、payload 版本、發布頁與測試。

- 以國巨 Q2 文章與固定 viewport 量測，修改前從文章根節點到第一個一般正文 section，在
  390×844 約為 5,485px（約 6.5 個 viewport），1280×900 約為 3,686px。新手任務卡現在保留原
  「開始讀三句重點」，並另提供次要「直接讀第一節」；accessible name 逐字包含第一個既有 H2，
  行動只定位 `topicReaderSectionItems(article)[0]`，焦點優先落在同節既有問題定位卡、再退回 H2。
  正式筆記、多空小作文、事件錨點與沒有一般正文的文章仍不顯示；中間三句重點、角色、路線、
  字典、摘要與查核附錄都保留原 DOM 順序。這是版面距離與導覽行為，不是理解率或學習成效統計。
- 國巨前三句改為先說公司公布了哪些數字、TWSE 月營收如何核對、毛利改善由哪些正負因素共同
  構成，以及現有證據還不能證明全面漲價、滿載或長約；「為什麼重要」改成「先拆成長來源 →
  再看 AI 占比 → 最後確認證據邊界」三段。另留
  `editorial:plain_language_wave148_yageo_beginner_opening` transition；閱讀時間因字數由 14 增為
  15 分鐘，沒有改動該文來源、主張、可信度、monitor、族群、學習路線或圖譜。
- `learningPathVersion: 100 → 101`。獨立 Python 與 Node JSON 路徑各自核對 274 篇文章：ID、順序與
  candidate radar 完全相同，只有 `topic-MI-2026-07-30-YAGEO-Q2-EARNINGS-CALL` 的 sections、
  summary、reading mission／boundary、搜尋文字與衍生閱讀時間改變。再排除該文、reader-only
  版本及 route station 的衍生分鐘數後，前後 payload 完全相同，canonical SHA-256 兩邊都是
  `2a6c86b52e3c6ece9925392417bd40e831a9bd59604c363723f1343d5f52aa9a`。
- Darwin 25.5.0 arm64、Python 3.11.11、UTF-8 預設環境執行 528 tests 全綠；qual notes、
  ...
  `1f645276f5f2bf7b27c2c017517baf9c3c2122b588606c0ec2ec3fdafe6c2c63`。
- 修改前後手機／桌機文章與正文定位畫面存於
  `tmp/research-learning-audit-2026-08-11-wave148-topic-body-bridge/`。固定 Browser viewport 不是實機；
  本輪未做真實新手理解測試、長期記憶測試、VoiceOver／TalkBack、200%／400% zoom 或完整實體
  鍵盤巡覽，因此不宣稱學習成效已提升或完整無障礙合規。

## 研究雷達先告訴讀者現在能做什麼 — 2026-08-11

**策略權重、tier 條件、regime 門檻、`IS_CUTOFF`、研究 source／claim／monitor、文章主張、
group／route／phase／graph／edge 關係、active radar／selection log、候選 rank／status、公司證據與
投資判斷零變動**；本次只把既有文章／圖譜行動前移到雷達題目後，並為 watch／deferred 題補上
固定 reader-only 導讀；同步研究維護契約、方法清單、README、payload 版本、發布頁與測試。

- 以同一張已升格候選與 390×844 viewport 比較。修改前，「閱讀這題的文章」在卡片約第 910px，
  必須先滑過起點、下一步、三個名詞與三個族群問句；修改後「這題現在怎麼讀」在卡片約第 208px
  出現，文章行動前移到約第 288px，同時保留同一 article、23 分鐘閱讀時間與 graph。第三張
  `watch` 候選原本沒有文章也沒有可採取行動，現在於卡片約第 247px 明說目前沒有文章，先用原
  三個族群問句分清責任並等待下一份證據；`deferred` 題明說目前不投入完整研究。這是同畫面與
  DOM 的資訊架構量測，不是理解率、完成率或學習成效統計。
- 每張卡只保留一組主要行動；390px 已升格卡為 1 個 article 與 1 個 graph 按鈕，watch／deferred
  卡沒有虛構入口。從前移按鈕開文後，手機起點仍為 `radar`、逐字保留雷達第 1 題與原白話問題；
  返回後同一卡片位於約 110px、焦點回到同一 candidate。320、390、780、884、1280、1440px 的
  `documentElement.scrollWidth` 均等於 viewport 寬度；780px 以下為單欄且按鈕至少 44px 高，
  884px 以上維持文字／行動雙欄。
- `learningPathVersion: 99 → 100`。獨立 Python 與 Node JSON 路徑各自核對 274 篇文章與 8 個候選；
  排除 reader-only 版本後，current 與本輪修改前 payload、候選順序、rank、status、article／graph
  與 group 映射完全相同；Python canonical SHA-256 兩邊都是
...
- Darwin 25.5.0 arm64、Python 3.11.11、UTF-8 預設環境執行 528 tests 全綠；qual notes、
  ...
  SHA-256 一致：`index.html`
  `82bf2fb334fb40351ce56d2448f4fec2b32594e74b927281a2463fad4652e943`、`research.html`
  `969db28f5343c07ca05d7ec5e28d563815b15ba0d03de1a7d11227937f5139ea`。
- 修改前後手機可讀題、手機待證題與桌機畫面存於
  `tmp/research-learning-audit-2026-08-11-wave147-radar-reader-brief/`。固定 Browser viewport 不是實機；
  本輪未做真實新手理解測試、長期記憶測試、VoiceOver／TalkBack、200%／400% zoom 或完整實體
  鍵盤巡覽，因此不宣稱學習成效已提升或完整無障礙合規。

## 讀完一站後，先說清楚這篇替整條問題補上什麼 — 2026-08-11

**策略權重、tier 條件、regime 門檻、`IS_CUTOFF`、研究 source／claim／monitor、文章主張、
`readingMission`、group／route／phase／graph／edge 關係、active radar／selection log、公司證據與
投資判斷零變動**；本次只在正式下一站卡逐字重用目前文章第一個既有重點，並修正桌機完成段的
等高拉伸；同步研究維護契約、方法清單、README、reader-only payload 版本、發布頁與測試。

- 以同一篇「800VDC 功率半導體鏈」與 1280×900 viewport 檢查「從這篇接著學」。修改前，左側
  自我檢查卡被右側下一站卡強制拉到約 507px 高，內容下方形成大塊空白；右側只重顯整條系統問題
  與「這一站／下一站」名稱，看不到剛讀完文章替整體問題建立的第一個框架。修改後，並排容器改為
  `align-items:start`，自我檢查卡約 195px、下一站卡約 602px，各自維持真實內容高度；下一站卡在
  系統問題後逐字顯示本篇 `readingMission.keyPoints[0]`，並以 article ID 與 index `0` 對回來源。
  這是同一畫面與 DOM 的版面／資訊架構觀察，不是理解率、完成率或學習成效統計。
- 按「繼續第 2/9 站」後，下一篇仍逐字帶入上一站 graph label、同一 key point、目前站 label 與
  問題；返回後焦點回到原按鈕，系統問題與本篇重點都仍存在。320、390、780、884、1280、1440px
  的 `documentElement.scrollWidth` 均等於 viewport 寬度；320／780／884px 為單欄，1280／1440px
  保留雙欄且兩卡不再互相拉伸。
- `learningPathVersion: 98 → 99`。獨立 Python 與 Node JSON 路徑各自核對 274 篇文章、4 條正式
  route 與 25 個正式站點；25/25 站都有非空第一個既有 key point。排除 reader-only 版本後，
  current 與本輪修改前 payload 完全相同，兩邊 canonical SHA-256 都是
  `af97e43694a980da1d0cec9b4e77c838784abda7beabe6818f38dbf9f11aa205`。
- Darwin 25.5.0 arm64、Python 3.11.11、UTF-8 預設環境執行 528 tests 全綠；qual notes、
  leading hypotheses、research queue、knowledge graph、research radar 與 method audit 六項 lint
  均為 exit 0；research queue 保留既有 11 個新鮮度／映射 warning。連續兩次 dashboard build
  SHA-256 一致：`index.html`
  `82bf2fb334fb40351ce56d2448f4fec2b32594e74b927281a2463fad4652e943`、`research.html`
  `7e791afbc61fa2a015019305f56e6c8ddd04c26af9c05e2a563a7ee985c3d687`。
- 修改前後手機／桌機與 320px 畫面存於
  `tmp/research-learning-audit-2026-08-11-wave146-completion-bridge/`。固定 Browser viewport 不是實機；
  本輪未做真實新手理解測試、長期記憶測試、VoiceOver／TalkBack、200%／400% zoom 或完整實體
  鍵盤巡覽，因此不宣稱學習成效已提升或完整無障礙合規。

## 從系統問題進文章後，首屏仍記得整條路線要回答什麼 — 2026-08-11

**策略權重、tier 條件、regime 門檻、`IS_CUTOFF`、研究 source／claim／monitor、文章主張、
group／route／phase／graph／edge 關係、active radar／selection log、公司證據與投資判斷零變動**；
本次只把正式 route 已有的 `question` 帶進文章首尾與手機起點摘要，並同步研究維護契約、方法
清單、README、reader-only payload 版本、發布頁與測試。

- 以固定 390×844 實走「第一次來 → 供電與散熱系統問題 → 第一站文章」。修改前，入口已顯示
  「電力如何送進 AI 機櫃，產生的熱又如何被帶走？」，但文章首屏只剩「系統問題 · 供電與
  散熱」，整條路線正在回答的問題要到文章底部才再次出現。修改後，文章首屏與底部返回卡逐字
  沿用同一 `route.question`，並以 `data-origin-route-id="power-cooling"` 對回同一條正式路線；
  不從文章題名、正文、族群或相似度另寫問題。這是同一 viewport 與狀態的資訊架構觀察，不是
  理解率、完成率或學習成效統計。
- 320×844 仍預設收合起點摘要，卡高約 48px；展開後約 99px，可讀到完整系統問題。390×844
  預設展開、卡高約 87px；780px 維持行動版摘要，884px 與 1280px 改用桌機完整起點卡。320、
  390、780、884、1280px 的 `documentElement.scrollWidth` 都等於 viewport 寬度，沒有水平
  溢位。返回後 hash 為 `#maturity`、焦點回到 `power-cooling` route；直接 article deep link
  仍沒有 origin 卡，返回文字維持「返回研究清單」。
- `learningPathVersion: 97 → 98`。獨立 Python 與 Node JSON 路徑各自核對 274 篇文章與 4 條正式
  route；排除 reader-only 版本後，current 與 HEAD payload 完全相同，兩邊 canonical SHA-256
  都是 `af97e43694a980da1d0cec9b4e77c838784abda7beabe6818f38dbf9f11aa205`。
- Darwin 25.5.0 arm64、Python 3.11.11、UTF-8 預設環境執行 528 tests 全綠；qual notes、
  leading hypotheses、research queue、knowledge graph、research radar 與 method audit 六項 lint
  均為 exit 0；research queue 保留既有 11 個新鮮度／映射 warning。連續兩次 dashboard build
  SHA-256 一致：`index.html`
  `82bf2fb334fb40351ce56d2448f4fec2b32594e74b927281a2463fad4652e943`、`research.html`
  `4074362622a29f01bc6b70a007e7908d302dd3ff1ea633fdbb28a369c9f62741`。
- 修改前矩陣入口、同狀態手機前後比較、320px 展開與桌機完整起點存於
  `tmp/research-learning-audit-2026-08-11-wave145-matrix-start/`。固定 Browser viewport 不是實機；
  本輪未做真實新手理解測試、長期記憶測試、VoiceOver／TalkBack、200%／400% zoom 或完整實體
  鍵盤巡覽，因此不宣稱學習成效已提升或完整無障礙合規。
