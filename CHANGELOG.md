...
- **絕對報酬軸**。2026-07-24：相對強勢（真強／蓄勢·外資佈局／強但過熱）23 檔裡
  ...
  三個逐檔介面（分層帶 pill、族群內個股表格、時間尺度卡）都在相對分數旁並列
  絕對報酬，上層 tier 但絕對為負者加 ↓；族群卡加「絕對 −10.4% · 1/10 檔漲」。
  導言檔數由當日資料現算，0 檔時整句消失。
...
- 新增契約測試：`test_absolute_return_contract.py` 10 條、`test_time_lens_contract.py`
  13 條、`test_dashboard_js_behaviour.py` 6 條。其中 12 條做過突變檢查（破壞後確實
  失敗）；JS 那 6 條改用 node 實際執行模板抽出的函式，不再只比對原始碼字串。
  釘住的實際坑：payload 漏欄、`D.allStocks` 投影欄名與 payload 不一致導致警語靜默
  空掉、`toFixed(0)` 把 −0.4% 印成「-0%」、`−0.0` 被印成綠色的「+0.0%」。
  全套 285 tests OK（Windows 11、python.org CPython 3.12.10、預設 cp950、
  未設 `PYTHONUTF8`/`PYTHONIOENCODING`）。
